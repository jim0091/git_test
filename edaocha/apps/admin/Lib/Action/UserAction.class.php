...
//      dump($listData);
        ...
        $this->pageButton[] = array('title' => '导出Excel', 'onclick' => 'admin.exportUser()');
...
//        dump($this->savePostUrl);
//        dump($this->pageKeyData);
    ...
    private function _initUserListAdminMenu($type = null)
    {
        ...
        // $this->pageTab[] = array('title'=>'在线用户列表','tabHash'=>'online','url'=>U('admin/User/online'));
        $this->pageTab[] = array('title' => '周星用户', 'tabHash' => 'starUser', 'url' => U('admin/User/starUser'));
        $this->pageTab[] = array('title' => L('PUBLIC_ADD_USER_INFO'), 'tabHash' => 'addUser', 'url' => U('admin/User/addUser'));
        // 搜索选项的key值
        // $this->searchKey = array('uid','uname','email','sex','department','user_group',array('ctime','ctime1'));
        $this->searchKey = array('uid', 'uname', 'email', 'mobile', 'state','sex', 'user_group', 'user_category', array('ctime', 'ctime1'),'province','city','vsource','code');
//        $this->searchKey = array('uid', 'uname', 'email', 'mobile', 'state','sex', 'user_group', 'user_category', array('ctime', 'ctime1'),'ltime_h','ltime_m','ltime_s','stime_h','stime_m','stime_s','province','city','vsource','code');
        // 针对搜索的特殊选项
        $this->opt['sex'] = array('0' => L('PUBLIC_SYSTEMD_NOACCEPT'), '1' => L('PUBLIC_MALE'), '2' => L('PUBLIC_FEMALE'));
//        $this->opt['ltime_h'] = M('hour')->getField('id,hour');
//        $this->opt['ltime_m'] = M('minute')->getField('id,minute');
//        $this->opt['ltime_s'] = M('second')->getField('id,second');
//        $this->opt['stime_h'] = M('hour')->getField('id,hour');
//        $this->opt['stime_m'] = M('minute')->getField('id,minute');
//        $this->opt['stime_s'] = M('second')->getField('id,second');
        $this->opt['identity'] = array('0' => L('PUBLIC_SYSTEMD_NOACCEPT'), '1' => L('PUBLIC_PERSONAL'), '2' => L('PUBLIC_ORGANIZATION'));
        //$this->opt['user_group'] = array_merge(array('0'=>L('PUBLIC_SYSTEMD_NOACCEPT')),model('UserGroup')->getHashUsergroup());
        ...
        $where1['pid'] = 0;
        $pro = M('area')->where($where1)->getField('area_id,title');
        $pro[0] = '不限';
        $this->opt['province'] = $pro;//选择省
        if(!empty($_POST['province'])){
           $map = array();
           $map['pid'] = $_POST['province'];
           $city = D('area')->field('area_id,title')->where($map)->select();
           foreach ($city as $k => $c) {
               $this->opt['city'][$c['area_id']] = $c['title'];
           }
        }
        $this->opt['city'][0] =  '不限';//选择市
//        $this->opt['vsource'] = array('1' => '');//来源选项
        $this->opt['vsource'] = array('0'=>'不限','B站' => 'B站','云音乐'=>'云音乐','1'=> '非采集用户');//来源选项
        $this->opt['code'] = array('0' => L('PUBLIC_SYSTEMD_NOACCEPT'), '1' => '是', '2' => '否');//是否使用邀请码
        //$this->opt['department_id'] = model('Department')->getHashDepartment();

        // 列表key值 DOACTION表示操作
        switch (strtolower($type)) {
            case 'index':
                $this->pageKeyList = array('uid', 'uname', 'is_code','phone', 'user_group', 'location', 'is_audit', 'is_active', 'is_init', 'ctime', 'reg_ip','device', 'DOACTION','state');
                break;
            case 'dellist':
...
//                $this->pageKeyList = array('uid', 'uname', 'phone', 'user_group', 'location', 'is_audit', 'is_active', 'is_init', 'ctime', 'reg_ip', 'disable_time', 'DOACTION');
                $this->pageKeyList = array('uid', 'uname', 'phone', 'user_group', 'location', 'ctime', 'reg_ip', 'disable_time', 'device','DOACTION');
                break;
            case 'pending':
                $this->pageKeyList = array('uid', 'uname', 'location', 'ctime', 'reg_ip', 'DOACTION');
                break;
            case 'staruser':
                $this->pageKeyList = array('uid', 'uname', 'ctime', 'reason', 'set_order','display_order','popularity','change', 'DOACTION');
                break;
        }
...
//            $listData['data'][$k]['uname'] = '<a style="color:#3589F1" href="'.U('admin/User/editUser', array('tabHash' => 'editUser', 'uid' => $v['uid'])).'">'.$v['uname'].'</a>'.$userGroupIcon[$v['uid']].' <br/>'.$v['email'].' '.$userTagString;
            $listData['data'][$k]['uname'] = '<a style="color:#3589F1" href="'.U('admin/User/editUser', array('tabHash' => 'editUser', 'uid' => $v['uid'])).'">'.$v['uname'].'</a>'.$userGroupIcon[$v['uid']];
            //是否使用邀请码
            $where['uid'] = $v['uid'];
            $res = D('register_bind')->where($where)->find();
            if($res){
                $where2['id'] = $res['code_id'];
                $code=D('register_code')->where($where2)->find();
                if($code){
                    $listData['data'][$k]['is_code'] = $code['code'];//是否使用邀请码
                }else{
                    $listData['data'][$k]['is_code'] = '邀请码不存在';
                }
            }else{
                $listData['data'][$k]['is_code']="未使用";
            }
            $listData['data'][$k]['ctime'] = date('Y-m-d H:i:s', $v['ctime']);
            // 屏蔽部门信息，若要开启将下面的注释打开
            ...
            switch (strtolower($type)) {
                case 'index':
                    ...
                    $listData['data'][$k]['DOACTION'] .= ' - <a href="'.U('admin/User/editStarUser', array('tabHash' => 'starUser', 'uid' => $v['uid'])).'">设置周星用户</a>';
                    // $listData['data'][$k]['DOACTION'] .= '<a href="'.U('admin/User/viewIP',array('tabHash'=>'viewIP','uid'=>$v['uid'])).'">查看IP</a>';
                if($v['state'] && mb_strlen($v['state'],'utf-8')>15){
                    $v['state'] = mb_substr($v['state'],'0','15','utf-8').'...';
                }
                    $listData['data'][$k]['state'] = ($v['state']) ?'<a style="color:green" href="'.U('admin/User/editState',array('tabHash' => 'editState', 'uid' => $v['uid'])).'">'.$v['state'].'</a>':'<a href="'.U('admin/User/editState',array('tabHash' => 'editState', 'uid' => $v['uid'])).'">设置备注</a>';//备注
                    break;
                case 'pending':
                    // 操作数据
...
//            $listData['data'][$k]['uname'] = '<a href="'.U('admin/User/editUser', array('tabHash' => 'editUser', 'uid' => $v['uid'])).'">'.$v['uname'].'</a>'.$userGroupIcon[$v['uid']].' <br/>'.$v['email'].' '.$userTagString;
            $listData['data'][$k]['uname'] = '<a href="'.U('admin/User/editUser', array('tabHash' => 'editUser', 'uid' => $v['uid'])).'">'.$v['uname'].'</a>'.$userGroupIcon[$v['uid']];
            $listData['data'][$k]['ctime'] = date('Y-m-d H:i:s', $v['ctime']);
            $listData['data'][$k]['identity'] = ($v['identity'] == 1) ? L('PUBLIC_PERSONAL') : L('PUBLIC_ORGANIZATION');
            $listData['data'][$k]['is_active'] = ($v['is_active'] == 1) ? '<span style="color:#2AB284;cursor:auto;">'.L('SSC_ALREADY_ACTIVATED').'</span>' : '<a href="javascript:void(0)" onclick="admin.activeUser(\''.$v['uid'].'\',1)" style="color:red">'.L('PUBLIC_NOT_ACTIVATED').'</a>';
            $listData['data'][$k]['is_audit'] = ($v['is_audit'] == 1) ? '<span style="color:#2AB284;cursor:auto;">'.L('PUBLIC_AUDIT_USER_SUCCESS').'</span>' : '<a href="javascript:void(0)" onclick="admin.auditUser(\''.$v['uid'].'\',1)" style="color:red">'.L('PUBLIC_AUDIT_USER_ERROR').'</a>';
    ...
    public function setDisableUser()
    {
        ...
        if ($result) {
            //记录操作日志
            LogRecord('admin_system', 'setDisableUser', array('name' => $uid, 'k1' => '禁用用户'), true);
    ...
    public function doUpdateUser(array $data = array())
    {
        ...
        // # 判断手机号码是否可以修改
        if ($phone and !$model->isChangePhone($phone, $uid)) {
            $this->error('当前手机号码已存在');
        }
        // # 判断用户邮箱是否可以修改
        if ($email and !$model->isChangeEmail($email, $uid)) {
            $this->error('当前邮箱已存在');
        }
        // # 判断用户性别
        ...
        //记录操作日志
        LogRecord('admin_system', 'doUpdateUser', array('name' => $uid, 'k1' => '编辑用户'), true);
    ...
    public function doTrueDeleteUser()
    {
        ...
        $info = model('User')->getUserInfo($_POST['id']);
        ...
        if (!$result) {
            ...
            //记录操作日志
            LogRecord('admin_system', 'doTrueDeleteUser', array('name' => $info['uname'], 'k1' => '彻底删除用户'), true);
        }
        exit(json_encode($return));
    }
    ...
    public function category()
    {
        //根据当前标签ID查询父级ID
        if($_GET['cid']){
            $info = model('UserCategory')->field('pid')->where('user_category_id='.$_GET['cid'])->find();
            if($info['pid']){
                $this->assign('p_id',$info['pid']);
            }else{
                $this->assign('p_id',$_GET['cid']); //zhl,用于添加修改删除完成后继续展开当前分类
            }
        }elseif($_GET['p_id']){
    ...
    public function doAddVerifyCategory()
    {
        ...
        if (D('user_verified_category')->where($data)->find()) {
            ...
            if (D('user_verified_category')->add($data)) {
                ...
                $return['data'] = '添加成功';
                //记录操作日志
                LogRecord('admin_system', 'doAddVerifyCategory', array('name' => t($_POST['title']), 'k1' => '新增认证分类'), true);
            } else {
                $return['status'] = 0;
                $return['data'] = '添加失败';
            }
    ...
    public function doEditVerifyCategory()
    {
        ...
        if (D('user_verified_category')->where($data)->find()) {
            ...
            if (D('user_verified_category')->where('user_verified_category_id='.$user_verified_category_id)->save($data) !== false) {
                ...
                $return['data'] = '编辑成功';
                //记录操作日志
                LogRecord('admin_system', 'doEditVerifyCategory', array('name' => $user_verified_category_id, 'k1' => '修改认证分类'), true);
            } else {
                $return['status'] = 0;
                $return['data'] = '编辑失败';
            }
    ...
    public function delVerifyCategory()
    {
        $user_verified_category_id = intval($_POST['user_verified_category_id']);
        //根据ID查询信息
        $info = D('user_verified_category')->field('title')->where('user_verified_category_id='.$user_verified_category_id)->find();
        if (D('user_verified_category')->where('user_verified_category_id='.$user_verified_category_id)->delete()) {
            $return['status'] = 1;
            $return['data'] = '删除成功';
            //记录操作日志
            LogRecord('admin_system', 'delVerifyCategory', array('name' => $info['title'], 'k1' => '删除认证分类'), true);
        } else {
            $return['status'] = 0;
            $return['data'] = '删除失败';
        }
    ...
    public function doOfficialAddUser()
    {
        ...
        if ($result) {
            ...
            //记录操作日志
            LogRecord('admin_system', 'doOfficialAddUser', array('name' => $uids, 'k1' => '添加推荐用户'), true);
            $this->success('操作成功');
        } else {
            $this->error('操作失败');
        }
    ...
    public function doRemoveOfficialUser()
    {
        ...
        if (empty($ids)) {
            ...
            if ($result) {
                ...
                $res['data'] = '操作成功';
                //记录操作日志
                LogRecord('admin_system', 'doRemoveOfficialUser', array('name' => $ids, 'k1' => '移除推荐用户'), true);
            } else {
                $res['status'] = 0;
                $res['data'] = '操作失败';
            }
    ...
    public function editStarUser()
    {
        $_REQUEST['tabHash'] = 'starUser';
        $this->_initUserListAdminMenu();
        // 列表key值 DOACTION表示操作
        $uid = intval($_REQUEST['uid']);
        $this->pageKeyList = array('uid', 'uname', 'reason', 'display_order','popularity');

        $this->savePostUrl = U('admin/User/doEditStarUser',array('uid'=>$uid));

        $userInfo = model('User')->getUserInfo($uid);

        $starUser = model('UserStar')->getUserStarInfo($uid);
        $starUser['uid'] = $userInfo['uid'];
        $starUser['uname'] = $userInfo['uname'];

        $this->displayConfig($starUser);
    }
    ...
    public function doEditStarUser()
    {
        $uid = intval($_REQUEST['uid']);

        empty($uid) && $this->error('请选择用户');
        ...
        if(model('UserStar')->hasStar($uid)){
            model('UserStar')->editStarUser($uid,$data);
        }else{
            model('UserStar')->addStarUser($uid,$data);
        }
    ...
    public function starUser()
    {
        ...
        $this->_initUserListAdminMenu('staruser');
        $this->pageButton[] = array('title' => '周星用户还原', 'onclick' => 'admin.revert()');
        $listData = model('UserStar')->getStarUserList();
        foreach ($listData['data'] as &$value){
            $value['set_order'] = '<a class="ico_top" href="javascript:;" onclick="admin.starUp(\''.$value['display_order'].'\',\''.$value['uid'].'\')"></a><a class="ico_btm" href="javascript:;" onclick="admin.starDown(\''.$value['display_order'].'\',\''.$value['uid'].'\')"></a>';
            $value['popularity'] = '<input  id="change_'.$value['uid'].'" type="text"  value="'.$value['popularity'].'" onchange="admin.popularityChange(\''.$value['uid'].'\')"></a>';
            if($value['change'] == 0){
                $value['change'] ="上升";
            }elseif($value['change'] == 1){
                $value['change'] ="下降";
            }elseif($value['change'] == 2){
                $value['change'] ="不变";
            }
        }
        //dump($listData);
    ...
    public function delStarUser()
    {
        ...
        if (empty($ids)) {
            ...
            // 删除操作
            $result = model('UserStar')->delStarUser($ids);
            // 返回结果集
            if ($result) {
                $res['status'] = 1;
                $res['data'] = '操作成功';
    ...
    public function city()
    {
        ...
        if (empty($area_id)) {
            $this->ajaxReturn('','省份不限',0);
        } else {
            $map['pid'] = $area_id;
            $res = M('area')->where($map)->select();
            if($res){
                $data = '<option value="0">'.L('PUBLIC_SYSTEMD_NOACCEPT').'</option>';
                foreach ($res as &$value){
                $data.='<option value="'.$value['area_id'].'">'.$value['title'].'</option>';
                }
                $this->ajaxReturn($data,'已找出城市',0);
            }else{
                $this->ajaxReturn('','下级城市不存在',0);
            }
        }
    }
    /**
    ...
    public function exportUser(){
        ...
        if(!empty($_GET['sex'])){
            $map['sex'] = intval($_GET['sex']);
        }

        if(!empty($_GET['province'])){
            $map['province'] = intval($_GET['province']);
        }
        if(!empty($_GET['city'])){
            $map['city'] = intval($_GET['city']);
        }
        // 注册时间判断，
        if (!empty($_GET['from_ctime_0']) && !empty($_GET['from_ctime_1'])) {
            // 时间区间条件
            $map['ctime'] = array(
                'BETWEEN',
                array(
                    strtotime($_GET['from_ctime_0']),
                    strtotime($_GET['from_ctime_1']),
                ),
            );
        } elseif (!empty($_GET['from_ctime_0'])) {
            // 时间大于条件
            $map['ctime'] = array(
                'GT',
                strtotime($_GET['from_ctime_0']),
            );
        } elseif (!empty($_GET['from_ctime_1'])) {
            // 时间小于条件
            $map['ctime'] = array(
                'LT',
                strtotime($_GET['from_ctime_1']),
        ...
        if(!empty($_GET['vsource'])){
            if($_GET['vsource'] == 1){
                $map['vsource'] = array('EXP','IS NULL');
            }else{
                $map['vsource'] = t($_GET['vsource']);
            }
        }
//         用户组信息过滤
        if (!$_GET['uid']) {
            if (!empty($_GET['user_group'])) {
                   $group_uid = getSubByKey(D('user_group_link')->where('user_group_id='.intval($_GET['user_group']))->findAll(), 'uid');           if($group_uid){
//                    $map['uid'] = array('in', $group_uid,);
                    $arr = $group_uid;
                }
            }
            // 用户标签过滤
            if (!empty($_GET['user_category'])) {
                $title = D('user_category')->where('user_category_id='.intval($_GET['user_category']))->getField('title');
                ...
                if ($group_uid) {
                        //$map['uid'] = array('in', array_intersect($group_uid, $tag_uid),);
                            $arr =array_intersect($group_uid, $tag_uid);
                } else {
                          //$map['uid'] = array('in', $tag_uid,);
                            $arr = $tag_uid;
                }
            }
        }
        //是否使用邀请码
        $cod = D('register_bind')->field('uid')->select();
        foreach ($cod as &$v){
            $code_arr[] = $v['uid'];
        }
        if($arr){
            if($_GET['is_code']==1){
                $map['uid'] = array(' in',array_intersect($arr, $code_arr));
            }else if($_GET['is_code']==2){
                $map['uid'] = array('in ',array_diff($arr,$code_arr));
            }else if($_GET['is_code']==0){
                $map['uid'] = array('in',$arr);
            }
        }else{
            if($_GET['is_code']==1){
                $map['uid'] = array('in',$code_arr);
            }else if($_GET['is_code']==2){
                $map['uid'] = array('not in ',$code_arr);
            }else if($_GET['is_code']==0){
        ...
        // 查询数据
        $order = '`uid` DESC';
        $list = M('user')->where($map)->order($order)->findAll();
//        dump($list);die;
        // 数据组装
        $userGroupHash = array();
        $uids = array();
        foreach ($list as $k => $v) {
               ...
               $uids[] = $v['uid'];
               $list[$k]['user_group'] = &$userGroupHash[$v['uid']];
        }
        ...
        foreach($list as &$v){
                if (!empty($v['user_group'])) {
                $group = model('UserGroup')->getUserGroup($v['user_group']);
                $group_name = array();
                foreach ($group as $g) {
                    $group_name[] = $g['user_group_name'];
                }
                 $v['user_group'] = implode('<br/>', $group_name);
             }
         }
         //格式化数据输出
        $data = array();
        foreach ($list as &$value) {
            ...
            $new_val['uname'] = $value['uname'];
            $new_val['phone'] = $value['phone'];
            $where['uid'] = $value['uid'];
            $res = M('register_bind')->where($where)->find();
            if($res){
                $where1['id'] = $res['code_id'];
                $code=D('register_code')->where($where1)->find();
                if($code){
                    $new_val['is_code'] = $code['code'];//是否使用邀请码
                }else{
                    $new_val['is_code'] = '邀请码不存在';
                }
            }else{
                $new_val['is_code'] = '未使用邀请码';
            }
            $new_val['user_group'] = $value['user_group'];
            $new_val['location'] = $value['location'];
            $new_val['ctime'] = date('Y-m-d H:i:s',$value['ctime']);
            $new_val['reg_ip'] = $value['reg_ip'];
            $new_val['device'] = $value['device'];
            $new_val['state'] = $value['state'];
            $data[] = $new_val;
        }
//       dump($data);die;
        //创建对象
        ...
        //表头数组
        $tableheader = array('用户ID','用户名','电话','邀请码','用户组','地区','注册时间','注册IP','设备','备注');
        //工作表设置
    ...
    public function starUp()
    {
        $uid = $_POST['uid'];
        if (empty($uid)) {
            $this->ajaxReturn('','星级用户不存在',0);
        } else {
            $display_order = $_POST['display_order'];
            if($display_order==1){
                $this->ajaxReturn('','已是第一，不可再提升',0);
            }else{
                $where['display_order'] = $display_order-1;
                $find = D('user_star')->where($where)->find();//判断上一个排序是否有人
                if($find){
                    $data['display_order'] = $find['display_order']+1;
                    $where['uid'] = $find['uid'];
                    $data['change'] = 1;
                    D('user_star')->where($where)->save($data);//有人的话，该人的星级下降1个
                }
                $map['uid'] = $uid;
                $data['display_order'] = $display_order-1;
                $data['change'] = 0;
                $res = D('user_star')->where($map)->save($data);
                if($res){
                    $this->ajaxReturn('','提升星级成功',1);
                }else{
                    $this->ajaxReturn('','提升星级失败',0);
                }
            }
        }
    ...
    public function starDown()
    {
        ...
        if (empty($uid)) {
            ...
            $display_order = $_POST['display_order'];
            $where['display_order'] = array('GT',$display_order);
            $order = 'display_order asc';
            $find = D('user_star')->where($where)->order($order)->find();//查询是否存在比此用户大的最近排序
            if($find){
                if($find['display_order'] == ($display_order+1)){
                    $where1['uid'] = $find['uid'];
                    $data['display_order'] = $find['display_order']-1;
                    $data['change'] = 0;
                    D('user_star')->where($where1)->save($data);//如果该用户的排序比此用户大1，则该用户上升1
                }
                $map['uid'] = $uid;
                $data1['display_order'] = $display_order+1;
                $data1['change'] = 1;
                $res = D('user_star')->where($map)->save($data1);
                if($res){
                    $this->ajaxReturn('','降低排序成功',1);
                }else{
                     $this->ajaxReturn('','降低排序失败',0);
                }
            }else{
                $this->ajaxReturn('','已是最后，不可再下降',0);//没有的话，不可再降
            }
        }
    }
    ...
    public function popularity(){
        ...
        if (empty($uid)) {
            ...
            if(is_numeric($popularity)){
                ...
                $res=D('user_star')->where($where)->save($data);
                if($res){
                    $this->ajaxReturn('','修改人气值成功',1);
                }else{
                    $this->ajaxReturn('','人气值没有变化',0);
                }
            }else{
                $this->ajaxReturn('','请输入正确人气值',0);
            }
        }
    }
     ...
     * 周星用户变化还原
     */
    public function revertStar(){
        $res = D('user_star')->select();
        $num = count($res);
        $i = 0;
        foreach ($res as &$value){
            $map['id'] = $value['id'];
            $change = D('user_star')->where($map)->find();
            if($change['change'] != 2){
                $data['change'] = 2;
                $result = D('user_star')->where($map)->save($data);
                if($result){
                    $i++;
                }
            }else{
 ...
 */
    public function editState(){
        ...
        $this->_initUserListAdminMenu();
        // 列表key值 DOACTION表示操作
        $this->pageKeyList = array('uid','state','is_state');
//        $this->opt['type'] = array('2' => L('PUBLIC_SYSTEM_FIELD'));
        $this->opt['is_state'] = array('1' => '');
        $uid = intval($_REQUEST['uid']);
        $userInfo = model('User')->getUserInfo($uid);
        if (!$userInfo) {
            $this->error(L('PUBLIC_GET_INFORMATION_FAIL'));
        ...
        $this->assign('pageTitle', L('PUBLIC_EDIT_USER'));
        $this->savePostUrl = U('admin/User/updateState');
        $stateList=D('user_state')->findAll();
        $list='<div class="state" style="display: none;width:400px;float:right;margin-top:-272px;margin-right:500px;border: #dcdcdc solid 1px;text-align: center">';
        foreach($stateList as $key => $value){
            $list.='<p id="state_'.$value['state_id'].'" style="cursor:pointer;display: inline-block;margin-left: 8px;margin-right: 8px" onclick="admin.stateAdd(\''.$value['state_id'].'\')">'.$value['state'].'</p>';
        }
        $list.='</div>';
    ...
    public function updateState()
    {
        ...
        if(!empty($data['state'])){
            $where1['state']=$data['state'];
//            dump($where);die;
            $res = D('user_state')->where($where1)->find();
            if(empty($res)){
                D('user_state')->add($data);
            }
        }
        // # 清理用户缓存
        $model->cleanCache($uid);
        model('Cache')->rm('perm_user_'.$uid);
        $this->assign('jumpUrl', U('admin/User/editState', array(
            'uid'     => $uid,
            'tabHash' => 'editState',
        )));
        $this->success(L('PUBLIC_SYSTEM_MODIFY_SUCCESS'));
    ...
    {
        // 页面具有的字段，可以移动到配置文件中！！！
        $this->pageKeyList = array('state_id','state','count', 'DOACTION');
//        $this->searchKey = array('id', 'uid', 'fuid', 'state', array('ctime', 'ctime1'));
        $this->pageTab[] = array('title' => '备注列表', 'tabHash' => 'state', 'url' => U('admin/User/state'));
        $this->pageButton[] = array('title' => '增加备注', 'onclick' => 'admin.addState()');
        $this->pageButton[] = array('title' => '搜索备注', 'onclick' =>  "admin.fold('search_form')");
        $this->pageButton[] = array('title' => '导出备注Excel', 'onclick' =>  "admin.exportState()");
        $this->searchKey = array('state');
        $map=array();
        !empty($_POST['state']) && $map['state'] = array('like','%'.($_POST['state']).'%');
        $list = D('user_state')->where($map)->order('state_id DESC')->findPage(20);
        foreach ($list['data'] as &$value) {
            $state = $value['state'];
            $where['state'] = array('LIKE','%'.$state.'%');
            $value['count'] = D('user')->where($where)->count();
            $value['DOACTION'] = "<a href='".U('admin/User/stateEdit', array('state_id' => $value['state_id']))."'>".L('PUBLIC_EDIT').'</a>&nbsp;-&nbsp;';
            $value['DOACTION'] .= "<a href='javascript:void(0)' onclick=\"admin.delState(this,'{$value['state_id']}')\">".L('PUBLIC_STREAM_DELETE').'</a> ';
        }

        $this->_listpk = 'state_id';
        $this->allSelected = false;
...
//        ob_clean();
        $this->pageTab[] = array('title' => '增加备注', 'tabHash' => 'addState', 'url' => U('admin/User/addState'));
        $this->pageKeyList = array('state');
        $this->assign('pageTitle', L('PUBLIC_EDIT_USER'));
        $this->savePostUrl = U('admin/User/stateAdd');
//       $this->notEmpty = array('state');
//        $this->onsubmit = 'admin.editState(this)';
        $this->displayConfig();
    }
    ...
    public function stateAdd(){
        ...
        $data['state'] = $state;
        $model->add($data);
        $this->assign('jumpUrl', U('admin/User/state'));
        $this->success('增加成功');
    }
    ...
    public function stateEdit()
    {
        // 初始化用户列表管理菜单
        $this->pageTab[] = array('title' => '备注列表', 'tabHash' => 'state', 'url' => U('admin/User/state'));
        $this->pageKeyList = array('state_id', 'state');
        $this->opt['type'] = array('2' => L('PUBLIC_SYSTEM_FIELD'));
        $state_id = intval($_REQUEST['state_id']);
        $where['state_id'] = $state_id;
        $userInfo = D('user_state')->where($where)->find();
        if (!$userInfo) {
            $this->error(L('PUBLIC_GET_INFORMATION_FAIL'));
        }
        $this->assign('pageTitle', L('PUBLIC_EDIT_USER'));
        $this->savePostUrl = U('admin/User/stateUpdate');
//        $this->notEmpty = array('email', 'mobile', 'uname', 'user_group');
        //$this->onsubmit = 'admin.editState(this)';
        $this->displayConfig($userInfo);
    }
        ...
        public function stateUpdate()
        {
            ...
            $where['state_id'] = array('neq',$state_id);
            $where['state'] = $state;
            $res = $model->where($where)->find();
            if($res){
                $this->error('这个备注已存在');
            ...
            $data['state'] = $state;
            $model->where($where1)->save($data);
            $this->assign('jumpUrl', U('admin/User/stateEdit', array(
                'state_id'     => $state_id,
                'tabHash' => 'stateEdit',
            )));
            $this->success(L('PUBLIC_SYSTEM_MODIFY_SUCCESS'));
    ...
    public function delState()
    {
        $state_id = intval($_POST['state_id']);
        //根据ID查询信息
        if (D('user_state')->where('state_id='.$state_id)->delete()) {
            $return['status'] = 1;
            $return['data'] = '删除成功';
        } else {
            $return['status'] = 0;
    ...
    public function exportState(){
        ...
        // 查询数据
        $order = '`state_id` DESC';
        $list = D('user_state')->where($map)->order($order)->findAll();
//        dump($list);die;
        //格式化数据输出
        $data = array();
        foreach ($list as &$value) {
            //提现账号查询
            $new_val = array();
            $new_val['state_id'] = $value['state_id'];
            $new_val['state'] = $value['state'];
            $state = $value['state'];
            $where['state'] = array('LIKE','%'.$state.'%');
            $value['count'] = D('user')->where($where)->count();
            $new_val['count'] = $value['count'];
            $data[] = $new_val;
        }
//       dump($data);die;
        //创建对象
        ...
        //表头数组
        $tableheader = array('备注ID','备注','绑定人数');
        //工作表设置
        $excel->setActiveSheetIndex( 0 );
        $excel->getActiveSheet()->freezePane('A2');
