...
class MedalAction extends AdministratorAction
{
    ...
    public function doAddMedal()
    {
        ...
        if (!$exist) {
            ...
            if ($res) {
                //记录操作日志
                LogRecord('admin_system', 'doAddMedal', array('name' => $name, 'k1' => '新增勋章'), true);
    ...
    public function doEditMedal()
    {
        ...
        if (!$exist) {
            ...
            if ($res) {
                //记录操作日志
                LogRecord('admin_system', 'doEditMedal', array('name' => $id, 'k1' => '修改勋章'), true);
                $this->assign('jumpUrl', U('admin/Medal/index'));
                $this->success('编辑成功');
            } else {
                $this->error('编辑失败');
...
// 		$this->pageButton[] = array( 'title' => '删除' , 'onclick' => "admin.deleteusermedal()" );
    ...
    public function doEditUserMedal()
    {
        ...
        if ($res) {
            //记录操作日志
            LogRecord('admin_system', 'doEditUserMedal', array('name' => $id, 'k1' => '修改用户勋章'), true);
    ...
    public function doAddUserMedal()
    {
        ...
        if ($medal_id) {
            ...
            if ($res) {
                ...
                //记录操作日志
                LogRecord('admin_system', 'doAddUserMedal', array('name' => '', 'k1' => '新增用户勋章'), true);
                $this->assign('jumpUrl', U('admin/Medal/userMedal'));
                $this->success('添加成功');
            } else {
    ...
    public function doDeleteMedal()
    {
        ...
        if ($res) {
            //记录操作日志
            LogRecord('admin_system', 'doDeleteMedal', array('name' => $ids, 'k1' => '删除勋章'), true);
            $return['status'] = 1;
            $return['data'] = '删除成功';
        } else {
            $return['status'] = 0;
