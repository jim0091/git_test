...
class MobileAction extends AdministratorAction
{
    ...
    public function doAddW3gSlideShow()
    {
        ...
        if ($id and D('w3g_slide_show')->where('`id` = '.$id)->field('id')->count()) {
            ...
            //记录操作日志
            LogRecord('admin_system', 'doSaveW3gSlideShow', array('name' => $id, 'k1' => '修改广场轮播'), true);
            $this->success('编辑成功！');
        }
        ...
        //记录操作日志
        LogRecord('admin_system', 'doAddW3gSlideShow', array('name' => '', 'k1' => '新增广场轮播'), true);
        // # 设置成功跳转地址
    ...
    public function delW3gSlideShwo()
    {
        ...
        //记录操作日志
        LogRecord('admin_system', 'delW3gSlideShwo', array('name' => $id, 'k1' => '删除广场轮播'), true);
        $this->success('删除成功！');
    }
