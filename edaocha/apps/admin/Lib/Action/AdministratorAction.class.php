...
class AdministratorAction extends Action
{
    ...
    public function _initialize()
    {
        ...
        if($GLOBALS['ts']['mid']!=1) {
            $map = array();
            $map['rule'] = $this->pageKey;
            $permission_node = D('permission_node')->where($map)->find();
            if($permission_node){
                if (!CheckPermission('core_admin', $this->pageKey)) {
                    //$this->assign('jumpUrl', SITE_URL);
                    $this->assign('message', '您没有权限访问');
                    $this->display(THEME_PATH.'/no_power.html');exit;
                }
            }
        }
        ...
        //除了超级管理员，菜单访问权限
//        if($GLOBALS['ts']['mid']!=1){
//            $disableMenu = array('Home_logs','Home_logsArchive','Home_schedule','Home_newschedule','Tool_cleancache',
//                'Home_cacheConfig','Tool_backup','Tool_index','Upgrade_check','Config_site',
//                'Config_nav','Config_footNav','Config_guestNav','Config_navAdd','Config_register','Config_invite',
//                'Config_getInviteAdminList','Config_inviteLink','Config_inviteEmail','Config_feed','Config_email','Config_sms',
//                'Config_cloudattach','Config_cloudimage','Config_attachimage','Config_attach','Global_credit',
//                'Config_charge','Config_area','Config_feedVideo','Global_creditType','Global_creditUser',
//                'Global_creditLevel','Config_ZBcharge','Config_ZB_config','User_profileCategory','User_addProfileField',
//                'User_addProfileCategory','Content_attachRec','Content_video_config',
//                'Department_index','Apps_setPermNode','Config_setSeo','Config_waterMark','User_profile',
//                'Content_attach','Apps_index','Apps_install','Addons_index',
//                'Mobile_w3gLogo','Mobile_setting','Application_socket','Application_jpush','UserGroup_index','LuckDraw_add');
//            if (in_array(MODULE_NAME.'_'.ACTION_NAME, $disableMenu))
//            {
//                $this->assign('message', '你没有权限访问');
//                $this->display(THEME_PATH.'/no_power.html');exit;
//                //$this->error('你没有权限访问');exit;
//            }elseif(MODULE_NAME.'_'.ACTION_NAME=='Addons_admin'){
...
//        dump($pageKeyData);
    ...
    public function savePageConfig()
    {
        ...
        if (model('Xconfig')->pageKey_lput('pageKey', $data)) {
            LogRecord('admin_config', 'editPagekey', array('name' => $key.'-'.$title, 'k1' => L('PUBLIC_ADMIN_EDIT_PEIZHI')), true);
            $this->success();
        } else {
            $this->error();
        }
    ...
    public function saveConfigData()
    {
        ...
        LogRecord('admin_config', 'editDetail', array('name' => $key.'-'.$title, 'k1' => L('PUBLIC_ADMIN_EDIT_EDTAIL_PEIZHI')), true);       // 保存修改编辑详细数据
