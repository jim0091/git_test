...
class UserGroupAction extends AdministratorAction
{
    ...
    public function index()
    {
        ...
        $list = model('UserGroup')->findPage(20);
//       dump($list);
         ...
         //   dump($_POST);exit;
            ...
            if ($res) {
                if($_POST['user_group_id']){
                    //记录操作日志
                    LogRecord('admin_system', 'saveUsergroup', array('name' => $_POST['user_group_id'], 'k1' => '修改用户组'), true);
                }else{
                    //记录操作日志
                    LogRecord('admin_system', 'addUsergroup', array('name' => $_POST['user_group_name'], 'k1' => '新增用户组'), true);
                }
...
//      $icons = array('-1'=>'无');
    ...
    public function delgroup()
    {
        ...
        //查询用户组名称
        $info = model('UserGroup')->getUserGroup($_POST['gid']);
        ...
        //记录操作日志
        LogRecord('admin_system', 'delgroup', array('name' => $info['user_group_name'], 'k1' => '删除用户组'), true);
        echo json_encode($return);
        exit();
    }
}
