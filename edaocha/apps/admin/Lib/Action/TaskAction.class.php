...
// 		}
    ...
    public function doEditTask()
    {
        ...
        if ($res) {
            //记录操作日志
            LogRecord('admin_system', 'doEditTask', array('name' => intval($_REQUEST['task_id']), 'k1' => '修改任务'), true);
    ...
    public function doEditReward()
    {
        ...
        //记录操作日志
        LogRecord('admin_system', 'doEditReward', array('name' => $tasklevel, 'k1' => '修改任务奖励'), true);
    ...
    public function taskConfig()
    {
        if ($_POST) {
            ...
            //记录操作日志
            LogRecord('admin_system', 'taskConfig', array('name' => '', 'k1' => '修改任务配置'), true);
            $this->success('保存成功');
        }
