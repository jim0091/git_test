...
class GlobalAction extends AdministratorAction
{
    ...
    public function doAddCreditType()
    {
        ...
        if ($res) {
            //记录操作日志
            LogRecord('admin_system', 'doAddCreditType', array('name' => $name, 'k1' => '新增积分类型'), true);
    ...
    public function doEditCreditType()
    {
        ...
        if ($res) {
            //记录操作日志
            LogRecord('admin_system', 'doEditCreditType', array('name' => $_POST['id'], 'k1' => '修改积分类型'), true);
            $db_prefix = C('DB_PREFIX');
            $model = M('');
    ...
    public function doDeleteCreditType()
    {
        ...
        if ($res) {
            //记录操作日志
            LogRecord('admin_system', 'doDeleteCreditType', array('name' => $delName, 'k1' => '删除积分类型'), true);
            $db_prefix = C('DB_PREFIX');
            $model = M('');
    ...
    public function doAddCredit()
    {
        ...
        if ($res) {
            //记录操作日志
            LogRecord('admin_system', 'addCredit', array('name' => $name, 'k1' => '新增积分规则'), true);
            // 清缓存
    ...
    public function doEditCredit()
    {
        ...
        if ($res !== false) {
            //记录操作日志
            LogRecord('admin_system', 'editCredit', array('name' => $id, 'k1' => '修改积分规则'), true);
            // 清缓存
            F('_service_credit_rules', null);
            $this->assign('jumpUrl', U('admin/Global/credit'));
            $this->success('保存成功');
    ...
    public function doDeleteCredit()
    {
        ...
        $_LOG['type'] = '2';
        $data[] = '全局 - 积分配置 - 积分规则 ';
        $data[] = $list = M('credit_setting')->where($map)->findAll();
        $_LOG['data'] = serialize($data);
        $_LOG['ctime'] = time();
        M('AdminLog')->add($_LOG);
        $name = '';
        ...
        if ($res) {
            //记录操作日志
            LogRecord('admin_system', 'delCredit', array('name' => $name, 'k1' => '修改积分规则'), true);
            // 清缓存
            F('_service_credit_rules', null);
            echo 1;
        } else {
    ...
    public function doCreditUser()
    {
        ...
        M('AdminLog')->add($_LOG);
        //记录操作日志
        LogRecord('admin_system', 'doCreditUser', array('name' => t($_POST['uId']), 'k1' => '设置用户积分'), true);
        $this->success('保存成功');
    }
    ...
    public function setCreditLevel()
    {
        if (!empty($_POST)) { //添加&编辑积分类型
           $res = model('Credit')->saveCreditLevel($_POST);
            //记录操作日志
            LogRecord('admin_system', 'setCreditLevel', array('name' => $_POST['name'], 'k1' => '设置积分等级'), true);
