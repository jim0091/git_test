...
/*		$this->opt ['photo_open'] = array (
    ...
    public function sensitive()
    {
        ...
        // 搜索字段配置
        $this->searchKey = array('word');
        $this->pageButton[] = array('title'=>'搜索敏感词', 'onclick'=>"admin.fold('search_form')");
        $this->pageButton[] = array('title' => '新增敏感词', 'onclick' => 'admin.setSensitiveBox()');
        $this->pageButton[] = array('title'=>'删除', 'onclick'=>"admin.rmSensitive()");

        !empty($_POST['word']) && $map['word'] = t($_POST['word']);
        ...
        $this->_listpk = 'sensitive_word_id';
        $this->displayList($listData);
    }
    ...
    public function doSetSensitive()
    {
        ...
        if (empty($id)) {
            ...
            //记录操作日志
            LogRecord('admin_system', 'addSensitive', array('name' => $word, 'k1' => '新增敏感词'), true);
        } else {
            $result = model('SensitiveWord')->setSensitiveWord($word, $replace, $type, $cid, $this->mid, $id);
            //记录操作日志
            LogRecord('admin_system', 'saveSensitive', array('name' => $id, 'k1' => '修改敏感词'), true);
        }
        $res = array();
        if ($result) {
    ...
    public function doRmSensitive()
    {
        ...
        if(is_array($id)){
            $word = '';
            foreach($id as $v){
                $info = model('SensitiveWord')->getSensitiveWord($v);
                $word .= $info['word'].'，';
            }
        }else{
            $info = model('SensitiveWord')->getSensitiveWord($id);
            $word = $info['word'];
        }
        ...
        if ($result) {
            //记录操作日志
            LogRecord('admin_system', 'delSensitive', array('name' => $word, 'k1' => '删除敏感词'), true);
            $res = array('status' => 1, 'info' => '操作成功');
        } else {
            $res = array('status' => 0, 'info' => '操作失败');
        }
    ...
    public function doNav()
    {
        ...
        if ($_GET['type']) {
            ...
            if (!$_GET['id']) {
                ...
                $rel = model('Navi')->add($map);
                //记录操作日志
                LogRecord('admin_system', 'addNav', array('name' => $map['navi_name'], 'k1' => '新增导航'), true);
            } else {
                $rel = model('Navi')->where('navi_id='.intval($_GET['id']))->save($map);
                //记录操作日志
                LogRecord('admin_system', 'saveNav', array('name' => intval($_GET['id']), 'k1' => '修改导航'), true);
            }
            $rel = true;
        }
    ...
    public function delNav()
    {
        ...
        if ($rel) {
            ...
            model('Navi')->cleanCache();
            //记录操作日志
            LogRecord('admin_system', 'delNav', array('name' => intval($_POST['id']), 'k1' => '删除导航'), true);
            $return = array(
                    'status' => 1,
    ...
    public function setEvent(){
        $this->pageKeyList = array('audit');
        $this->opt['audit'] = array('1' => '是', '0' => '否');
        $this->displayConfig();
    }
}
