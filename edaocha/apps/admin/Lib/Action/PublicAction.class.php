...
class PublicAction extends AdministratorAction
{
    ...
    public function doAddTreeCategory()
    {
        ...
        if ($result) {
            //记录操作日志
            LogRecord('admin_system', 'addTreeCategory', array('name' => $_POST['stable'].'表新增：'.$title, 'k1' => '新增分类'), true);
            $res['status'] = 1;
            $res['data'] = '添加分类成功';
    ...
    public function doUpTreeCategory()
    {
        ...
        if ($result) {
            //记录操作日志
            LogRecord('admin_system', 'saveTreeCategory', array('name' => $_POST['stable'].'表修改ID为：'.$cid, 'k1' => '修改分类'), true);
            $res['status'] = 1;
            $res['data'] = '编辑分类成功';
            S('api_discover_system', null);
        } else {
    ...
    public function rmTreeCategory()
    {
        ...
        //根据ID查询分类
        $info = model('CategoryTree')->setApp($app)->setTable($stable)->getCategoryInfo($cid);
        ...
        if ($result) {
            //记录操作日志
            LogRecord('admin_system', 'delTreeCategory', array('name' => $_POST['stable'].'表删除：'.$info['title'], 'k1' => '删除分类'), true);
            $res['status'] = 1;
            $res['data'] = $msg;
            S('api_discover_system', null);
        } else {
