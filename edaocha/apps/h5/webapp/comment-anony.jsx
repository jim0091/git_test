...
class CommentBox extends React.Component
{
  ...
  componentWillUnmount() {
    ...
    this.props = props;
		this.cacheKeyName = 'feed-comment-'+props.anonyId;
    let isCache = props.isCache;
      ...
      this.scroll = true;
		} else if (this.anonyId != props.anonyId) {
      this.scroll = false;
      this.anonyId = props.anonyId;
			$.ajax({
				url: buildURL('comment', 'getList'),
				type: 'POST',
				dataType: 'json',
				data: {
                    table:'aodou_video_data',
                    row_id:props.anonyId,
				},
			})
			.done(function(data) {
				this.state.befores = props.before;
	...
	render() {
		var anonyId = this.props.anonyId;
        ...
        <div style={styles.commentHr}></div>
				{this.state.befores.map(function(comment) {
					return [
						(<CommentItem key={guid()} comment={comment} feedId={anonyId} />),
						(<Divider inset={false} />)
					];
				})}
    ...
    if (this.scroll == true) {
      ...
      if (height == top || height - top < 10) {
        ...
        request
          ...
          .field('row_id', this.props.anonyId)
          .field('min_id', this.min)
          .field('num', 30)
          .end((error, ret) => {
            ret.body.forEach((comment) => {
              this.min = comment.comment_id;
              this.handleAppendCommentItem(comment, this.props.anonyId);
            });
            this.scroll = true;
          })
        ;
  ...
  handleAppendCommentItem(data, anonyId) {
    ...
    ReactDOM.render(
      (<MuiThemeProvider muiTheme={muiTheme}>
        ...
        >
          <CommentItem key={guid()} comment={data} feedId={anonyId} />
          <Divider inset={true} />
        </div>
      </MuiThemeProvider>),
      divDOM
