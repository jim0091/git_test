...
class CommentBox extends React.Component
{
  ...
  componentWillUnmount() {
    ...
    this.props = props;
		this.cacheKeyName = 'feed-comment-'+props.videoId;
    let isCache = props.isCache;
      ...
      this.scroll = true;
		} else if (this.videoId != props.videoId) {
      this.scroll = false;
	...
	render() {
        ...
        <div style={styles.commentHr}></div>
				{this.state.befores.map(function(comment) {
					return [
						(<CommentItem key={guid()} comment={comment} feedId={videoId} />),
						(<Divider inset={false} />)
					];
				})}
    ...
    if (this.scroll == true) {
      ...
      if (height == top || height - top < 10) {
        ...
        request
          ...
          .field('row_id', this.props.videoId)
          .field('min_id', this.min)
          .field('num', 30)
          .end((error, ret) => {
            ret.body.forEach((comment) => {
              this.min = comment.comment_id;
              this.handleAppendCommentItem(comment, this.props.videoId);
            });
            this.scroll = true;
          })
        ;
  ...
  handleAppendCommentItem(data, videoId) {
    ...
    ReactDOM.render(
      (<MuiThemeProvider muiTheme={muiTheme}>
        ...
        >
          <CommentItem key={guid()} comment={data} feedId={videoId} />
          <Divider inset={true} />
        </div>
      </MuiThemeProvider>),
      divDOM
