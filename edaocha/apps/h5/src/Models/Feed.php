...
class Feed extends Model
{
    ...
    public function commtentFeedCount($feed_id){
        if(empty($feed_id)){
            return false;
        }
        return M('comment')->where(array('row_id' => $feed_id, 'table' => 'feed'))->count();
    }
    ...
    public function getVideoAttribute()
    {
        ...
        if ($this->type != 'postvideo') {
            ...
            return array(
                'image' => $video_server.$this->data->feed_data_object->image_path,
                'src' => $video_server.$this->data->feed_data_object->video_path,
                'type' => 'ts',
            );
        }
    ...
    public function postvideo()
    {
        ...
        if ($data['row_type'] == 'vendor') {
            ...
            $video_config = model('Xdata')->get('admin_Content:video_config');
            $video_server = $video_config['video_server'] ? $video_config['video_server'] : SITE_URL;
            $data['image'] = $video_server.$_temp->image_path;
            $data['url'] = $video_server.$_temp->video_path;
        }
