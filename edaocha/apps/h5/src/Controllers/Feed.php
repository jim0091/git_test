...
class Feed extends Controller
{
    public function getFeedInfoAction()
    {
        ...
        $feed['user']['anonymous_name'] = $user_info['anonymous_name'];
        ...
        $feed['user']['anonymous_icon'] = $user_info['anonymous_icon'];//匿名头像
        ...
        $feed['commentNum'] = $info->commtentFeedCount($feedId);
        ...
        // 获取分享点赞列表（默认只取25条）
    public function feedDiggListAction()
    {
        $feedId = Common::getInput('feed_id');

        $info = Model\Feed::existent()->find($feedId);
    ...
    {
        list($max, $min, $time) = Common::getInput(array('max', 'min', 'time'));
        if($time >= 5){
            $feeds = array();
            $this->__json__($feeds);
        }
