...
class ChannelModel extends Model
{
    ...
    public function getDataWithCid($cid, $loadId, $limit, $order, $loadCount)
    {
        ...
        if ($order == 0) {
            ...
            $countmap['cy_feed.is_recommend'] = 1;
            $result = $this->field('cy_channel.*,cy_feed.comment_count')->join('cy_feed on cy_channel.feed_id = cy_feed.feed_id')->where($countmap)->order('comment_count DESC,feed_channel_link_id DESC')->findPage($limit * 4);
            //获取数据
            $limit = ($loadCount - 1) * $limit + ((intval($_REQUEST['p']) - 1) * $limit * 4).','.$limit;
            $map['cy_feed.is_recommend'] = 1;
            $data = $this->field('cy_channel.*,cy_feed.comment_count')->join('cy_feed on cy_channel.feed_id = cy_feed.feed_id')->where($map)->order('comment_count DESC,feed_channel_link_id DESC')->limit($limit)->findAll();
        }
