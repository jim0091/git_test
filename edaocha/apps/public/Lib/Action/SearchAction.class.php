    ...
    public function index()
    {
        ...
        if ($this->curType == 1) {     //搜索用户
            if ($this->key != '') {
                ...
                //搜索统计
                $count = $list['data']?count($list['data']):0;
                model('KeywordStatistic')->setKeywordStatistic($this->key,2,$count);
            }
            $this->display('person');
            ...
            if ($this->key != '') {
                ...
                //搜索统计
                $count = $list['data']?count($list['data']):0;
                model('KeywordStatistic')->setKeywordStatistic($this->key,0,$count);
            }
...
// 					$userGids = model('UserGroupLink')->getUserGroup($v['uid']);
    ...
    public function new_search(){
        ...
        if ($this->curType == 1) {
            $this->assign('k', $k);
            $this->display('feed_search');
        }else{
            $informationData = Subject::getInstance()->searchInformation($k);
            foreach ($informationData['data'] as &$value) {
                $value['logo'] = getImageUrlByAttachId($value['logo']) ?: '';
                if(mb_strlen($value['subject'],'utf-8') > 17){
                    $value['subject'] = mb_substr($value['subject'],0,17,'utf-8').'...';
                }
            }
            $this->assign('k', $k);
            $this->assign('informationData', $informationData);
            //搜索统计
            $count = $informationData['data']?count($informationData['data']):0;
            model('KeywordStatistic')->setKeywordStatistic($this->key,4,$count);
            $this->display('information_search');
        }
    }
}
