...
class ProfileAction extends Action
{
    ...
    public function index()
    {
        ...
        // 获取用户用户组信息
        $userGids = model('UserGroupLink')->getUserGroup($this->uid);
        //判断用户是否是评论点赞用户组
        if (in_array("10", $userGids[$this->uid])){
            $this->assign('show_comment_count', 1);
            //查询当前用户今天评论数和点赞数量
        ...
        //判断用户是否是分享统计用户组
        if (in_array("9", $userGids[$this->uid])){
            $this->assign('show_feed_count', 1);
            //查询当前用户今日分享数和被推荐数
            $start_time = strtotime(date('Y-m-d', time()));
            $end_time = strtotime(date('Y-m-d H:i:s', time()));
            $map = array();
            ...
            $recommend_map['uid'] = $user_info['uid'];
