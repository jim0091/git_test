...
class IndexAction extends Action
{
    ...
    public function home(){
        $this->assign('nav','0');
        $this->assign('downapp',model('Xdata')->get('admin_Config:appLink'));
        $this->display();
    }
    ...
    public function index(){
        ...
        if($categoryid == 0){            
            $this->assign('_title','奥豆pro_奥豆热门动态_发现好玩的事_奥豆');
            $this->assign('_description','搜罗散落于网络中的有趣内容，用户在这里发现会心一笑的内容，找到合拍的小伙伴儿。');
            $map['is_recommend'] = 1;
            $list =  model('Feed')->where($map)->order('feed_id DESC')->limit(50)->getAsFieldArray('feed_id');
            $list = model("Feed")->formatFeed($list, true);;
        }else{
            foreach ($category as $key => $value) {
                if($value['channel_category_id'] == $categoryid){
                    $catetitle = $value['title'];
                }
            }
            $list = model('Feed')->getChannelFeed($categoryid,50);
            $this->assign('_title','奥豆pro _奥豆'.$catetitle.'热门动态 _奥豆');
            $this->assign('_description','搜罗散落于网络中'.$catetitle.'的有趣内容，用户在这里发现会心一笑的内容，找到合拍的小伙伴儿。');
        }
        //热门活动查询
        $event = array();
        $num = 3;
        ...
        //热门文章查询
        $num = 4;
        $informationData = Subject::getInstance()->getHot($num,72);
    ...
    public function look(){
        ...
        $this->assign('category',$category);
        if($categoryid == 0){
            $list =  model('Feed')->order('feed_id DESC')->limit(10)->getAsFieldArray('feed_id');
            $list = model("Feed")->formatFeed($list, true);;
        }else{
            $list = model('Feed')->getChannelFeed($categoryid);
        }
    ...
    public function about(){
        $type = 5;
        //服务协议
        $service_html = model('Xdata')->get('admin_Application:agreement');
        $this->assign('service_html', $service_html['content']);
    ...
    public function event_list(){
        $this->assign('_title','奥豆活动_用户专属的兴趣圈子_奥豆');
        $this->assign('_description','各类有趣、好玩、实用的线上线下活动举行，并且将线上沟通发展为线下活动，从而打造用户专属的长期发展的兴趣圈子。');
        $this->assign('nav','2');//索引class
    ...
    public function event_info(){
        ...
        $this->assign('_title',$data['name'].'_奥豆');
        $this->assign('_description',substr(htmtocode(t($data['content'])),0,60));
        /* 地区 */
    ...
    public function information_list(){
        ...
        foreach ($cate_data as $key => $value) {
            if($value['id'] == $cate){
                $catetitle = $value['name'];
            }
        }
        empty($catetitle) && $catetitle = '全部';        
        $this->assign('_title',$catetitle.'文章_'.$catetitle.'热门内容_奥豆');
        $this->assign('_description','奥豆提供关于'.$catetitle.'的文章给大家阅读，让大家及时了关于{健康}的热门内容。');
        $limit = isMobile() ? 10 : 20;
        $informationData = Subject::getInstance()->setCate($cate)->setStatus(1)->getList($limit);
        foreach ($informationData['data'] as &$value) {
            $value['logo'] = getImageUrlByAttachId($value['logo']) ?: '';
            if(mb_strlen($value['abstract'],'utf-8')>60){
                $value['abstract'] = mb_substr($value['abstract'],0,60,'utf-8').'...';
            }
        }
        $this->assign('informationData', $informationData);
        //精彩热文
        $num = 4;
        $hot_information = Subject::getInstance()->getHot($num,72);
        foreach ($hot_information as &$value) {
            $value['logo'] = getImageUrlByAttachId($value['logo']) ?: '';
        }
        $this->assign('hot_information', $hot_information);

        if(isMobile()){
            $this->assign('min',$informationData['data'][count($informationData['data'])-1]['id']);
            $this->display('information_list_mobile');
        }else{
            $this->display();
        }
    }
     ...
     *
     */
    public function information_list_more(){
        $cate = intval($_GET['cate']);
        $max_id = intval($_GET['max_id']);
        $informationData = Subject::getInstance()->setCate($cate)->getList(20,$max_id);
        foreach ($informationData['data'] as &$value) {
            $value['logo'] = getImageUrlByAttachId($value['logo']) ?: '';
        }
        $this->assign('informationData', $informationData);
...
//        $cate = intval($_GET['cate']);
//        $this->assign('cate', $cate);
        $dataInfo = Subject::getInstance()->setId($id)->getSubject();
        $this->assign('_title',$dataInfo['subject'].'_奥豆');
        $this->assign('_keywords',$dataInfo['tag']);
        $this->assign('_description',$dataInfo['abstract']);
        Subject::getInstance()->setHits($dataInfo['hits'] + 1)->update();
        $dataInfo['hits'] || $dataInfo['hits'] = 0;
        //正则搜索和替换
        $patterns = array('/src="/');
        $replacements = array('src="https://v2.edaocha.net');
        $dataInfo['content'] = preg_replace($patterns, $replacements, $dataInfo['content']);

        foreach ($cate_data as $key => $value) {
            if($value['id'] == $dataInfo['cid']){
                $catetitle = $value['name'];
                $this->assign('catetitle', $catetitle);
            }
        }
        $this->assign('information_info', $dataInfo);
        //相关文章查询
        $InformationList = Subject::getInstance()->setCate($dataInfo['cid'])->getInformationList(4,$id);
        foreach ($InformationList['data'] as &$value) {
            $value['logo'] = getImageUrlByAttachId($value['logo']) ?: '';
        }
        $this->assign('InformationList', $InformationList);
        if(isMobile()){
            $this->display('information_info_mobile');
        }else{
            $this->display();
        }
