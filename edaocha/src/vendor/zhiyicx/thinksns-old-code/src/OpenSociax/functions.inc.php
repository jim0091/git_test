...
function showTreeCategory($data, $stable, $left, $delParam, $level = 0, $ext = '', $times = 1, $limit = 0)
{
    ...
    foreach ($data as $val) {
        ...
        if (empty($delParam)) {
            $html .= '<a href="javascript:;" onclick="admin.rmTreeCategory('.$val['id'].', \''.$stable.'\', \''.$val['pid'].'\');">删除</a>';
        } else {
            $html .= '<a href="javascript:;" onclick="admin.rmTreeCategory('.$val['id'].', \''.$stable.'\', \''.$val['pid'].'\', \''.$delParam['app'].'\', \''.$delParam['module'].'\', \''.$delParam['method'].'\');">删除</a>';
        }
...
/**
 * @param $begintime 开始日期 2017-02-01
 * @param string $endtime 结束日期 2017-02-20
 * @return int 返回时间戳
 *
 *
 * 生成某个范围内的随机时间
