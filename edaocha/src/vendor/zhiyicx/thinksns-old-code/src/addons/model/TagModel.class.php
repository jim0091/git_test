...
class TagModel extends Model
{
     ...
     * @return string
     *
     * 根据用户ID查询他的标签及对应的标签分类
     */
    public function getUserTags($row_ids){
        $map = array();
        $map['a.row_id'] = $row_ids;
        $map['a.app'] = 'public';
        $map['a.table'] = 'user';
        $table = $this->tablePrefix.'app_tag AS a LEFT JOIN '.$this->tablePrefix.'tag AS b ON a.tag_id = b.tag_id';
        $result = $this->Table($table)->field('b.tag_id,b.name')->where($map)->select();
        if($result){
            //根据标签名称查询分类信息
            foreach($result as $key=> &$v){
                $cateMap = array();
                $cateMap['title'] = $v['name'];
                $cateMap['pid'] = array('neq','0');
                $cate = D('user_category')->field('user_category_id,pid')->where($cateMap)->find();
                if($cate){
                    $p_name = D('user_category')->field('title')->where('user_category_id = '.$cate['pid'])->find();
                    $v['cate_name'] = $p_name['title'];
                }else{
                    unset($result[$key]);
                }
            }
