...
class FeedDiggModel extends Model
{
    ...
    public function addDigg($feed_id, $mid)
    {
        ...
        $data ['is_anonymous'] = $feed['is_anonymous'];
        $data['cTime'] = time();
        $res = $this->add($data);
        ...
        if ($res) {
            ...
            //判断是否是匿名点赞
            if($feed['is_anonymous']){
                model('UserData')->updateKey('anonymous_unread_digg', 1, true, $feed['uid']);
            }
    ...
    public function setDiggCache($uid, $feedId, $type = 'add')
    {
        $key = 'user_digg_'.$uid;
        $data = S($key);
        if (!$data) {
