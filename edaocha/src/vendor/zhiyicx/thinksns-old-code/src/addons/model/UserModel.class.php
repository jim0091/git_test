...
class UserModel extends Model
{
    ...
    public function getUserList($limit = 20, array $map = array(), $order = '`uid` DESC')
    {
        ...
        if (isset($_POST)) {
//            !empty($_POST['vsource']) && $map['vsource'] = array('neq','');
//            !empty($_POST['vsource']) && $map['vsource'] = t($_POST['vsource']);
            if(!empty($_POST['vsource'])){
                if($_POST['vsource'] == 1){
                    $map['vsource'] = array('EXP','IS NULL');
                }else{
                    $map['vsource'] = t($_POST['vsource']);
                }
            }
//            dump($map);die;
            ...
            $_POST['state'] &&$map['state'] = array('LIKE','%'.t($_POST['state']).'%');
            ...
            // 邀请码判断，
            $data = M('register_bind')->field('uid')->select();
            foreach ($data as &$v){
                $uid_arr[] = $v['uid'];
            }
            if($_POST['is_code']==1){
                $map['uid'] = array(' in',$uid_arr);
            }else if($_POST['is_code']==2){
                $map['uid'] = array('not in ',$uid_arr);
            }
            !empty($_POST['province']) && $map['province'] = intval($_POST['province']);//选择省
            !empty($_POST['city']) && $map['city'] = intval($_POST['city']);//选择市
            // 注册时间判断，ctime为数组格式
...
//        echo $this->getLastSql();
    ...
    public function addUser(array $user)
    {
        ...
        if (!$regmodel->isValidName($user['uname'])) {
    ...
    private function _getUserInfo(array $map, $field = '*')
    {
        ...
        if (!$user) {
            ...
            $user['space_link'] = "<a href='".$user['space_url']."' target='_blank' uid='{$user['uid']}'>".$user['uname'].'</a>';
...
// 			$authenticate = array();
    ...
    public function trueDeleteUserCoreData($uid_array)
    {
        ...
        //如果用户有匿名名称，则取消匿名
        $anonymous_list = D('user_anonymous')->field('anonymous_id')->where($map)->select();
        if($anonymous_list){
            foreach($anonymous_list as $v){
                $save = array();
                $save['use_count'] = array('exp','use_count-1');
                $save['id'] = $v['anonymous_id'];
                D('anonymous')->save($save);
            }
        }
