...
class UserGroupLinkModel extends Model
{
     ...
     * 分页查询用户组为8的用户
     */
    public function getFeedManage($map,$count=10){
        $list = D()->table('`'.C('DB_PREFIX').'user_group_link` AS ul LEFT JOIN `'.C('DB_PREFIX').'user` AS u ON u.`uid` = ul.`uid`')
            ->field('ul.uid,u.uname')
            ->where($map)
            ->order('u.uid DESC')
            ->findPage($count);
     ...
     * 查询全部用户组为8的用户
     */
    public function getFeedManageAll($map){
        $list = D()->table('`'.C('DB_PREFIX').'user_group_link` AS ul LEFT JOIN `'.C('DB_PREFIX').'user` AS u ON u.`uid` = ul.`uid`')
            ->field('ul.uid,u.uname')
            ->where($map)
            ->order('u.uid DESC')
