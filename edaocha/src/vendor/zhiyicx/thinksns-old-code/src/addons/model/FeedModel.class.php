...
class FeedModel extends Model
{
    ...
    protected $fields = array('feed_id','is_top', 'uid', 'type', 'app', 'app_row_id', 'app_row_table', 'publish_time', 'is_del', 'from', 'comment_count', 'repost_count', 'comment_all_count', 'digg_count', 'is_repost', 'is_audit', 'latitude', 'longitude', 'address', 'is_recommend', 'recommend_time','is_anonymous','recommend_top', '_pk' => 'feed_id');
    ...
    public function put($uid, $app = 'public', $type = '', $data = array(), $app_id = 0, $app_table = 'feed', $extUid = null, $lessUids = null, $isAtMe = true, $is_repost = 0)
    {
        ...
        $userGids = model('UserGroupLink')->getUserGroup($uid);
        if (in_array("9", $userGids[$uid])||in_array("7", $userGids[$uid])){
            $data['is_audit'] = 1;
        }else{
            //普通用户根据系统配置来判断
        ...
        if ($feed_id && $feed_data) {
            ...
            if($data['is_audit'] == 1){
                //CDN缓存预热
                if ($data['video_id']) {
                    $video_path = $data['video_path'] ? $data['video_path'] : $data['video_mobile_path'];
    ...
    {
        $order = !empty($order) ? $order : 'is_top DESC,feed_id DESC';
     ...
     * 获取分享列表,后台使用
     */
    public function getListAdmin($map, $limit = 10, $order = null)
    {

        $order = !empty($order) ? $order : 'a.is_top DESC,a.feed_id DESC';

//        $table = "{$this->tablePrefix}feed AS a LEFT JOIN {$this->tablePrefix}feed_data AS b ON a.feed_id = b.feed_id";
        $table = "{$this->tablePrefix}feed AS a LEFT JOIN {$this->tablePrefix}feed_data AS b ON a.feed_id = b.feed_id LEFT JOIN {$this->tablePrefix}channel AS c ON a.feed_id = c.feed_id";
        $feedlist = $this->table($table)->field('a.feed_id')->where($map)->order($order)->findPage($limit);
...
//		!empty($type) && $map['type'] = $type;
    ...
    private function __paseTemplate($_data)
    {
        ...
        $var['uid'] = $_data['uid'];
        $var['actor'] = "<a href='{$user['space_url']}' class='name' uid='{$user['uid']}'>{$user['uname']}</a>";
        ...
        if (empty($feed_id)) {
            ...
            if ($type == 'deleteFeed') {
                ...
                foreach ($feedArr as $sid) {
                    ...
                    $info_map = array();
                    $info_map['a.feed_id'] = $sid;
                    $data = $this->where($info_map)
                        ->table("{$this->tablePrefix}feed AS a LEFT JOIN {$this->tablePrefix}feed_data AS b ON a.feed_id = b.feed_id ")
                        ->find();
                    $fd = unserialize($data['feed_data']);
                    if (!empty($fd['attach_id'])) {
                        ...
                        foreach ($attach as $ak => $av) {
                            if ($data['type'] == 'postimage') {
                                $def = SITE_PATH.'/data/upload/'.$av['save_path'].$av['save_name'];
                                if(file_exists($def)){
                                    unlink($def);
                                }
                                $thumbInfo = getThumbImage($av['save_path'].$av['save_name'], 120, 120, true);
                                $img_120 = SITE_PATH.'/data/upload/'.ltrim($thumbInfo['src'], '/');
                                if(file_exists($img_120)){
                                    unlink($img_120);
                                }
                                $thumbInfo = getThumbImage($av['save_path'].$av['save_name'], 240);
                                $img_240 = SITE_PATH.'/data/upload/'.ltrim($thumbInfo['src'], '/');
                                if(file_exists($img_240)){
                                    unlink($img_240);
                                }
                                $thumbInfo = getThumbImage($av['save_path'].$av['save_name'], 740);
                                $img_740 = SITE_PATH.'/data/upload/'.ltrim($thumbInfo['src'], '/');
                                if(file_exists($img_740)){
                                    unlink($img_740);
                                }
                                $thumbInfo = getThumbImage($av['save_path'].$av['save_name'], 240, 240, true);
                                $img_240_240 = SITE_PATH.'/data/upload/'.ltrim($thumbInfo['src'], '/');
                                if(file_exists($img_240_240)){
                                    unlink($img_240_240);
                                }
                            }
                        }
                        //删除data数据
                        $del_data = $this->where('feed_id='.$sid)->table("{$this->tablePrefix}feed_data")->delete();
                    }
                }
                // 彻底删除分享
    ...
    public function doAuditFeed($feed_id)
    {
        ...
        if (empty($feed_id)) {
            ...
            $map['feed_id'] = is_array($feed_id) ? array('IN', $feed_id) : intval($feed_id);
            //CDN预热缓存
            $feed_list = $this->field('feed_id,type')->where($map)->select();
            foreach($feed_list as $v){
                /*if($v['type']=='postimage'){
                    $attach_map = array();
                    $attach_map['row_id'] = $v['feed_id'];
                    $attach_map['is_del'] = 0;
                    $imgList = model('Attach')->where($attach_map)->select();
                    if($imgList){
                        $i = 1;
                        ...
                        foreach($imgList as $img){
                            if($i<count($imgList)){
                                $objectPath .= "https://v2.edaocha.net/data/upload/".$img['save_path'].$img['save_name']."\n";
                            }else{
                                $objectPath .= "https://v2.edaocha.net/data/upload/".$img['save_path'].$img['save_name'];
                            }
                            $i++;
                ...
                if($v['type']=='postvideo'){
                    $video_info = D('video')->where('feed_id='.$v['feed_id'])->find();
                    if($video_info){
     ...
     * 通过分享并到热门
     */
    public function doAuditAndRecommendFeed($feed_id){
        $return = array('status' => '0');
        if (empty($feed_id)) {
            $return['data'] = '请选择分享！';
        } else {
...
// 		}
    ...
    // 获取指定频道下的分享
    public function getChannelFeed($cid, $limit = 10)
    {
        $map['status'] = 1;
     ...
     * 查询用户参与的微博
     */
    public function part_in_timeline($map){
        $Model = new Model();
        $count = $map['count']==''?10:$map['count'];
        $uid = $map['uid'];
        $is_anonymous = $map['is_anonymous']==''?1:$map['is_anonymous'];
        $max_id = $map['max_id'];
        $where = " 1=1";
        if($max_id){
            $where .= " AND feed_id < $max_id";
        }
        $sql = "SELECT feed_id from(

 SELECT row_id as feed_id from cy_comment where uid='$uid' and app='public' and `table`='feed' and is_del=0 and is_anonymous='$is_anonymous' GROUP BY row_id

 union all

 SELECT ct.row_id as feed_id from cy_comment_digg as cd INNER JOIN cy_comment as ct on ct.comment_id=cd.comment_id where cd.uid='$uid' and ct.is_anonymous='$is_anonymous' and ct.is_del=0

	union all

 SELECT feed_id FROM cy_feed_digg where uid='$uid' and is_anonymous='$is_anonymous'

) as f where $where GROUP BY feed_id ORDER BY feed_id DESC LIMIT $count";
        $feed_list = $Model->query($sql);
     ...
     * 获取分享列表,后台使用
     */
    public function getHostlist($map, $limit = 20, $order = null)
    {

        $order = !empty($order) ? $order : 'a.feed_id DESC';

        $table = "{$this->tablePrefix}feed AS a LEFT JOIN {$this->tablePrefix}channel AS b ON a.feed_id = b.feed_id";
        $hotlist = $this->table($table)->field('a.feed_id')->where($map)->order($order)->findPage($limit);
        $feed_ids = getSubByKey($hotlist['data'], 'feed_id');
        $hotlist['data'] = $this->getFeeds($feed_ids);
//        $feed_ids = getSubByKey($feedlist['data'], 'feed_id');
//        $feedlist['data'] = $this->getFeeds($feed_ids);
//        echo $this->getLastSql();
        return $hotlist;
