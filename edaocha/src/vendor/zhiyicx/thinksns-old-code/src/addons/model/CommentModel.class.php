...
class CommentModel extends Model
{
    ...
    protected $fields = array('type', 'comment_id', 'app', 'table', 'row_id', 'app_uid', 'uid', 'content', 'to_comment_id', 'to_uid', 'data', 'ctime', 'is_del', 'client_type', 'is_audit', 'storey', 'app_detail_url', 'app_detail_summary', 'client_ip', 'client_port', 'digg_count','is_anonymous');
...
/*             $storeyCount = $this->where("table='".$add['table']."' and row_id=".$data['row_id'].' and comment_id<'.$res)->count();
            ...
            if ($GLOBALS['ts']['mid'] != $add['app_uid'] && $add['app_uid'] != '' && $add['app_uid'] != $add['to_uid']) {
                ...
                if (!$notCount and $add['app'] == 'weiba') {
                    ...
                    //如果是匿名评论，则添加匿名评论未读消息
                    if($add['is_anonymous']){
                        model('UserData')->updateKey('anonymous_unread_comment', 1, true, $add['app_uid']);
                    }
                }
            }
            // 回复发送提示信息
            if (!empty($add['to_uid']) && $add['to_uid'] != $GLOBALS['ts']['mid']) {
                ...
                if (!$notCount and $add['app'] == 'weiba') {
                    ...
                    //如果是匿名评论，则添加匿名评论未读消息
                    if($add['is_anonymous']){
                        model('UserData')->updateKey('anonymous_unread_comment', 1, true, $add['to_uid']);
                    }
                }
            }
            // 加积分操作
...
/*       	    if (isset($uid) && !in_array($uid, array($c_v['app_uid'], $c_v['uid']))) {
    ...
    private function _escapeData($data)
    {
        ...
        $add['is_anonymous'] = intval($data['is_anonymous']);
        return $add;
    }
