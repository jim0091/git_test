...
class CommentWidget extends Widget
{
    ...
    public function render($data)
    {
        ...
        if($data['mustlist'] == 1){
            $content = $this->getCommentList('public','feed',$data['feed_id'],$data['num'],$data['tpl']);
            return $content;
        }
     ...
     * @return array
     */
    public function getCommentList($app_name = '',$table = '',$row_id = '',$num = 3, $tpl = '')
    {
        $map = array();
        $map['app'] = empty($app_name) ? t($_POST['app_name']) : $app_name;
        $map['table'] = empty($table) ? t($_POST['table']) : $table;
        $map['row_id'] = empty($row_id) ? intval($_POST['row_id']) : $row_id;
        if (!empty($map['row_id'])) {
            // 分页形式数据
            $var['limit'] = empty($num) ? 10 : $num;
            $var['order'] = 'DESC';
