        ...
        switch ($type) {
            case 'following': // 我关注的
                if (!empty($var['feed_key'])) {
                    ...
                    $where = ' a.is_audit=1  AND a.is_del = 0 AND a.is_anonymous = 0';
            ...
            case 'all': // 所有的 --正在发生的
                if (!empty($var['feed_key'])) {
                    ...
                    $where = ' is_audit=1  AND is_del = 0  AND is_anonymous = 0';
                    if ($var['loadId'] > 0) { // 非第一次
            ...
            case 'space': // 用户个人空间
                ...
                break;
            case 'channel':
                $where = ' c.is_audit=1 AND c.is_del = 0 AND c.is_anonymous = 0';
                if ($var['loadId'] > 0) { // 非第一次
            ...
            case 'getchannel':
                ...
                $list = D('ChannelFollow', 'channel')->getFollowingFeed($where, $this->limitnums, '', $var['channel_category_id']);*/
                global $tablePrefix;
                if($var['channel_category_id'] == 0){
                    $map['is_recommend'] = 1;
                    $map['is_del'] = 0;
                    $map['is_audit'] = 1;
                    $map['is_anonymous'] = 0;
                    $list = model('Feed')->getList($map,50,'recommend_time desc');
                }else{
                    $channel = D('channel')->getTableName();
                    $feed = D('feed')->getTableName();
                    $table = "`{$channel}` AS a LEFT JOIN `{$feed}` AS c ON a.feed_id = c.feed_id";
                    $ids = D()->table($table)->field('a.feed_id')->where(array('a.channel_category_id'=>$var['channel_category_id'],'a.status'=>1,'c.is_audit'=>1,'c.is_del'=>0))->order('feed_id DESC')->findPage(50);
                    $feedIds = getSubByKey($ids['data'], 'feed_id');
                    $map['feed_id'] = array('in',implode(',',$feedIds));
                    $list = model('Feed')->getList($map,50);
                }
                $content['count'] = $list['count'];
                break;
            case 'one':
                $where = ' is_audit=1 AND is_del = 0 AND AND is_anonymous = 0 feed_id = '.$var['feed_id'] ;
                // 设定可查看的全站分享总数，可以提高大数据量下的查询效率
                $max = null; //10000;
...
//              }
                ...
                $list = model('Feed')->getListNew($map, $this->limitnums, 'recommend_time', 'DESC'); //zhl排序方式recommend_time或者feed_id
                $content['count'] = $list['count'];
                break;
            case 'weiba': // 推荐
//                  if ($var ['maxId'] > 0) {
