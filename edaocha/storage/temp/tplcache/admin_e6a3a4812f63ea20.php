...
<link href="<?php echo APPS_URL;?>/admin/_static/admin_photo_layer.css" rel="stylesheet" type="text/css">
<script>
/**
 * 全局变量
 */
...
<script src="__THEME__/js/jquery.atwho.js"></script>
...
<div id="container" class="so_main">
  ...
  <div class="Toolbar_inbox">
    ...
    <?php if(is_array($pageButton)): ?><?php $i = 0;?><?php $__LIST__ = $pageButton?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$b): ?><?php ++$i;?><?php $mod = ($i % 2 )?><?php
        $dataArgs = '';
        if (is_array($b['data'])) {
      ...
      ?>
      <a onclick="<?php echo ($b["onclick"]); ?>" class="btn_a <?php echo ($b["class"]); ?>" id="<?php echo ($b["id"]); ?>" <?php echo ($dataArgs); ?>><span><?php echo ($b["title"]); ?></span></a>
      <?php unset($dataArgs); ?><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
  </div>
  <!-- END TOOLBAR -->
  <?php endif; ?>
...
<script type="text/javascript">
    admin.codeUser=function(uid,code){
        $.ajax({
            url: "<?php echo U('admin/User/codeUser');?>",
            dataType: "Json",
            type: "POST",
    ...
    $('#form_province').change(function () {
        ...
        if(area_id>0){
            $.ajax({
                url: "<?php echo U('admin/User/city');?>",
                dataType: "Json",
                type: "POST",
                data: 'area_id='+area_id,
                success: function (res) {
                    $('#form_city').html(res.data);
                // alert(res.info);
                }
            });
        }else{
            $('#form_city').html('<option value="0" selected="selected">不限</option>');
        }
    });
    //导出用户Excel
    admin.exportUser = function(){
        ...
        var user_group = $("#form_user_group").val();
        var user_category = $("#form_user_category").val();
        var from_ctime_0 = $("#from_ctime_0").val();
        var from_ctime_1 = $("#from_ctime_1").val();
        var is_code = $("input:radio[name='code']:checked").val();
        var province = $("#form_province").val();
        var city = $("#form_city").val();
        var vsource = $("#form_vsource").val();
        // var vsource = $("input:checkbox[name='vsource[]']:checked").val();
        // alert(is_code);
        var str_get = '&uid='+uid+'&uname='+uname+'&email='+email+'&mobile'+mobile+'&state='+state+'&sex='+sex+'&user_group='+user_group+'&user_category='+user_category+'&from_ctime_0='+from_ctime_0+'&from_ctime_1='+from_ctime_1+'&is_code='+is_code+'&province='+province+'&city='+city+'&vsource='+vsource;
        var url = "<?php echo U('admin/User/exportUser');?>";
        location.href = url + str_get;
    };

    //提高排序
    admin.starUp = function (display_order,uid) {
        // alert(display_order);
        // alert(uid);
        $.ajax({
            url: "<?php echo U('admin/User/starUp');?>",
            dataType: "Json",
            type: "POST",
            data: 'display_order='+display_order+'&uid='+uid,
            success: function (res) {
                if(res.status==1){
                    window.location.reload();
                }else{
    ...
    //降低排序
    admin.starDown = function (display_order,uid) {
        // alert(display_order);
        // alert(uid);
        $.ajax({
            url: "<?php echo U('admin/User/starDown');?>",
            dataType: "Json",
            type: "POST",
            data: 'display_order='+display_order+'&uid='+uid,
            success: function (res) {
    ...
    //修改人气值
    admin.popularityChange = function (uid) {
         var popularity=$("#change_"+uid).val();
         if(popularity==""){
             alert('请输入人气值');
             return;
         }
        $.ajax({
            url: "<?php echo U('admin/User/popularity');?>",
            dataType: "Json",
            type: "POST",
            data: 'popularity='+popularity+'&uid='+uid,
            success: function (res) {
                if(res.status==1){
                    window.location.reload();
                }else{
    ...
    //增加备注
    admin.addState = function(){
        location.href = U('admin/User/addState');
    };

    //删除备注
    admin.delState = function(obj,state_id){
        // alert(state_id);
        $.ajax({
            url: "<?php echo U('admin/User/delState');?>",
            dataType: "Json",
            type: "POST",
    ...
    //导出备注Excel
    admin.exportState = function(){
        var state = $("#form_state").val();
        // alert(state);
        var str_get = '&state='+state;
        var url = "<?php echo U('admin/User/exportState');?>";
        location.href = url + str_get;
    };
    //导出邀请码绑定Excel
    admin.codeExport = function () {
        var form_id = $('#form_id').val();
        var form_code = $('#form_code').val();
        var form_uid = $('#form_uid').val();
        var bind_time_0 = $('#from_bind_time_0').val();
        var bind_time_1 = $('#from_bind_time_1').val();
        var str_get = '&form_id=' +form_id+ '&form_code='+form_code+'&form_uid='+form_uid+'&bind_time_0='+bind_time_0+'&bind_time_1='+bind_time_1;
        var url = "<?php echo U('admin/RegisterCode/exportCode');?>";
        location.href= url + str_get;
    }
    // 修改内容频道
    // admin.channel=function(feed_id){
    //     var feed_id = feed_id;
    //     var channel = $("#change_"+feed_id).val();
    //     // alert(feed_id);
    //     // alert(channel);
    //     // return false;
    //     $.ajax({
    //         url: "<?php echo U('admin/Content/channelChange');?>",
    //         dataType: "Json",
    //         type: "POST",
    //         data: 'feed_id='+feed_id+'&channel='+channel,
    //         success: function (res) {
    //             if(res.status==1){
    //                 window.location.reload();
    //             }else{
