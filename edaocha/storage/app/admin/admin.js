...
admin.delStarUser = function(id){
    ...
    if(confirm( '确定要彻底删除选中帐号？' )){
        $.post(U('admin/User/delStarUser'),{id:id},function(msg){
            admin.ajaxReload(msg);
        },'json');
    }
};
...
admin.rmSensitive = function(id) {
    ...
    //    return false;
    //}
    var _id = ("undefined"== typeof(id)|| id=='') ? admin.getChecked() : id;
    if(_id ==''){
        ui.error('请选择要删除的信息');
        return false;
    }
    if (confirm('确定删除该信息')) {
        $.post(U('admin/Config/doRmSensitive'), {id:_id}, function(res) {
            if (res.status == 1) {
                ui.success(res.info);
                setTimeout(function() {
                    location.reload();
...
admin.setTk = function(url,field){
    ui.box.load(U(url)+'&setTk='+field);
};
admin.subjectMore = function(id){
    ui.box.load(U('Information/Admin/subjectMore')+'&id='+id);
};

//提现管理用到的JS
admin.doToCash = function(_id,action,title,type){
    var id = ("undefined"== typeof(_id)|| _id=='') ? admin.getChecked() : _id;
    if(id==''){
        ui.error(L('PUBLIC_SELECT_TITLE_TYPE',{'title':title,'type':type}));
        return false;
    ...
    if(confirm(L('PUBLIC_CONFIRM_DO',{'title':title,'type':type}))){
        $.post(U('admin/LuckDraw/'+action),{id:id},function(msg){
            admin.ajaxReload(msg);
        },'json');
    }
};
...
//匿名分类删除
admin.rm_anonymous_category = function(id) {
    var _id = ("undefined"== typeof(id)|| id=='') ? admin.getChecked() : id;
    if(_id ==''){
        ui.error('请选择要删除的分类');
        return false;
    }
    if (confirm('删除分类同时会删除下面的子类，确定要删除吗？')) {
        $.post(U('admin/Application/doRmAnonymousCategory'), {id:_id}, function(res) {
            if (res.status == 1) {
                ui.success(res.info);
                setTimeout(function() {
                    location.reload();
...
//匿名删除
admin.rm_anonymous = function(id) {
    var _id = ("undefined"== typeof(id)|| id=='') ? admin.getChecked() : id;
    if(_id ==''){
        ui.error('请选择要删除的匿名');
        return false;
    ...
    if (confirm('确定要删除吗？')) {
        $.post(U('admin/Application/doRmAnonymous'), {id:_id}, function(res) {
            if (res.status == 1) {
                ui.success(res.info);
                setTimeout(function() {
                    location.reload();
...
//奥豆视频查看更多信息
admin.aodou_video_more = function(id){
    ui.box.load(U('admin/Content/aodouVideoMore')+'&id='+id);
};


//奥豆视频删除
admin.rm_aodou_video = function(id) {
    var _id = ("undefined"== typeof(id)|| id=='') ? admin.getChecked() : id;
    if(_id ==''){
        ui.error('请选择要删除的视频');
        return false;
    }
    if (confirm('删除视频同时会删除下面的剧集，确定要删除吗？')) {
        $.post(U('admin/Content/doRmVideo'), {id:_id}, function(res) {
            if (res.status == 1) {
                ui.success(res.info);
                setTimeout(function() {
                    location.reload();
...
//奥豆剧集删除
admin.rm_aodou_video_data = function(id) {
    var _id = ("undefined"== typeof(id)|| id=='') ? admin.getChecked() : id;
    if(_id ==''){
        ui.error('请选择要删除的剧集');
        return false;
    ...
    if (confirm('确定要删除吗？')) {
        $.post(U('admin/Content/doRmVideoData'), {id:_id}, function(res) {
            if (res.status == 1) {
                ui.success(res.info);
                setTimeout(function() {
                    location.reload();
...
//广场置顶
admin.ContentTop = function(_id,action,title,type){
    var id = ("undefined"== typeof(_id)|| _id=='') ? admin.getChecked() : _id;
    if(id==''){
        ui.error(L('PUBLIC_SELECT_TITLE_TYPE',{'title':title,'type':type}));
        return false;
...
//热门置顶
admin.ContentRecommendTop = function(_id,action,title,type){
    var id = ("undefined"== typeof(_id)|| _id=='') ? admin.getChecked() : _id;
    if(id==''){
        ui.error(L('PUBLIC_SELECT_TITLE_TYPE',{'title':title,'type':type}));
        return false;
...
admin.delKeyword = function(id) {
    ...
    if (confirm('确定要删除吗？')) {
        $.post(U('admin/KeywordStatistic/doDelKeyword'), {id:_id}, function(res) {
            if (res.status == 1) {
                ui.success(res.info);
                setTimeout(function() {
                    location.reload();
...
admin.state=function(){
    if($("input:checkbox[name='is_state[]']").is(':checked')) {
        $('.state').css("display","block");
    }else{
        $('.state').css("display","none");
    }
}
admin.stateAdd=function (state_id) {
    var text = $('#state_'+state_id).text();
    $('#form_state').val($('#form_state').val()+" "+text);

}
// 周星用户还原设置
admin.revert=function(){
    if(confirm('确定将周星用户变化还原？')){
        location.href = U('admin/User/revertStar');
    }
}
...
admin.channel=function(feed_id){
    ...
    // return false;
    $.ajax({
        url: U('admin/Content/channelChange'),
        dataType: "Json",
        type: "POST",
        data: 'feed_id='+feed_id+'&channel='+channel,
        success: function (res) {
            if(res.status==1){
                $("#change_"+feed_id).html(res.data);
...
// 将热门动态下的视频推送到视频频道下
admin.sweepHot=function(){
    if(confirm('确定将视频推送到视频频道？')){
        location.href = U('admin/Content/sweepHot');
    }
}

admin.changeChannel=function (feed_id) {
    ui.box.load(U('admin/Content/changeChannel')+'&feed_id='+feed_id, "修改频道");
