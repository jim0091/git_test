...
//args['loadmore']   	= loadmore;新版本去掉加载功能
...
var feed_recommend = function(feedId, val) {
	$.post(U('public/Feed/feed_recommend'),{feed_id:feedId,val:val},function(){
		...
		switch (val) {
			case 0:
				$tmp.find('a[rel="feed_tj"]').html('推荐动态').attr('onclick', 'feed_recommend(' + feedId + ', ' + 1 + ')');
				break;
			case 1:
				$tmp.find('a[rel="feed_tj"]').html('取消推荐').attr('onclick', 'feed_recommend(' + feedId + ', ' + 0 + ')');
				break;
		}
