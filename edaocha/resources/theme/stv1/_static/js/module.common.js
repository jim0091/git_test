...
M.addModelFns({
    ...
    down_app:{
        ...
        click:function(){
            ui.box.load(U('public/Index/downApp'));
            return false;
        }
    },
    drop_menu_list: {
...
M.addEventFns({
    ...
    feedback:{//反馈意见
        click :function(){
            ui.box.load(U('public/Feedback/index'),'意见反馈');
            return false;
        },
    },
    feedsearch:{
        click : function(){
            var val = this.parentModel;
            var input = $(val).find('input').get(0);
            //window.location.href=U('public/feed/search')+'&key='+input.value;
            window.location.href=U('public/Search/index')+'&t=2&a=public&k='+input.value;
        }
    },
    ifsearch:{
        click : function(){
            var val = this.parentModel;
            var input = $(val).find('input').get(0);
            //window.location.href=U('public/information/search')+'&key='+input.value;
            window.location.href=U('public/Search/new_search')+'&t=2&a=public&k='+input.value;
        }
    },
    down_app:{
        click:function(){
    ...
    post_feedback:{
        click:function(){
            if("undefined" == typeof(this.feedback) || this.feedback == false){
                this.feedback = true
            }else{
                return false; //避免重复提交
            }
            var _this = this;
            var mini_editor = this.parentModel.childModels['post_feedback'];
            var textarea = $(mini_editor).find('textarea').get(0);
            var data = textarea.value;

            $.post(U('public/feedback/addfeedbackforweb'),{data:data},function(msg){
