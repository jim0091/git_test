...
//					_this.startNewLoop();
	...
	post_feed: function(_this, mini_editor, textarea, isbox , url) {	
        ...
        $('.btn-green-big').html('<span>发布中</span><span class="dotting"></span>');
		//alert(url);exit;
		// 发布发言
		$.post(url, {body:data, type:type, app_name:app_name,channel:channel, content:'', attach_id:attach_id,video_id:video_id,videourl:videourl, channel_id:channel_id, source_url:attrs.source_url, gid:attrs.gid}, function(msg) {
			...
			if(msg.status == 1) {
                ...
                $('.btn-green-big').html('<span>' + L('PUBLIC_SHARE_BUTTON') + '</span>');
			} else {
				ui.error(msg.data);
                $('.btn-green-big').html('<span>' + L('PUBLIC_SHARE_BUTTON') + '</span>');
			}
    ...
    switchVideo : function(id,app_row_id,type,host,flashvar,flashimg, width, height){
		var repost_str = '';
		if(app_row_id){
