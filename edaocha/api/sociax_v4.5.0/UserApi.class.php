...
class UserApi extends Api
{
    ...
    public function show()
    {
        ...
        $pvInfo = model('User')->field('pv')->where('uid='.$uid)->find();//个人访问量
        $user_info['pv'] = $pvInfo['pv'];
        ...
        $user_info['weibo_count'] = M('feed')->where(array('uid'=>$uid,'is_del'=>0,'is_anonymous'=>0))->count();
        $user_info['follower_count'] = t(intval($userInfo['user_data']['follower_count']));
        $user_info['following_count'] = t(intval(M('user_follow')->where(array('uid'=>$uid))->count()));
        //用户空间隐私判断
        ...
        $user_info['tags_new'] = model('Tag')->getUserTags($uid);
    ...
    public function get_user_info($uid)
    {
        ...
        if (!$user_info) {
            ...
            $user_anonymous = D('user_anonymous')
                ->field('a.bd_time,b.nickname,c.icon')
                ->where($map)
            ...
            $user_info['anonymous_bd_time'] = $user_anonymous['bd_time'];
            $user_info['anonymous_change_title'] = '每两天可以更新一次花名';
        ...
        $user_info['space_privacy'] = $privacy['space'];
        //根据绑定时间判断是否可以修改标识
        //$now = strtotime("-48 hours");
        $now = strtotime("-1 Minute");
        $bd_time = $user_info['anonymous_bd_time'];
        if($now>$bd_time){
            $user_info['anonymous_change'] = '1';
        }else{
            $user_info['anonymous_change'] = '0';
        }
     ...
     * 用户图片和视频集合
     */
    public function user_photo_video($uid_param){
        if ($uid_param) {
            $uid = $uid_param;
            $this->count = 20;
        } else {
        ...
        $map['a.uid'] = $uid;
        $map['a.type'] = array('in', "'postvideo','postimage'");
        $map['a.is_del'] = 0;
        ...
        $list = D()->table('`'.C('DB_PREFIX').'feed` AS a LEFT JOIN `'.C('DB_PREFIX').'feed_data` AS b ON a.`feed_id` = b.`feed_id`')->field('a.`feed_id`, a.`publish_time`, a.`type`, b.`feed_data`')->where($map)->order('feed_id DESC')->limit($count)->findAll();
        $video_config = model('Xdata')->get('admin_Content:video_config');
        $video_server = $video_config['video_server'] ? $video_config['video_server'] : SITE_URL;
        $photo_video_list = array();
        foreach ($list as $k => $value) {
            $tmp = unserialize($value['feed_data']);
            if($value['type']=='postvideo'){
                $res_val = array();
                $res_val['feed_id'] = $value['feed_id'];
                $res_val['type'] = $value['type'];
                $video_id = $tmp['video_id'];
                if ($video_id) {
                    $res_val['flashimg'] = $video_server.$tmp['image_path'];
//                    if ($tmp['transfer_id'] && !D('video_transfer')->where('transfer_id='.$tmp['transfer_id'])->getField('status')) {
//                        $res_val['transfering'] = 1;
//                    } else {
//                        $res_val['flashvar'] = $tmp['video_mobile_path'] ? $video_server.$tmp['video_mobile_path'] : $video_server.$tmp['video_path'];
//                    }
                    $res_val['flashvar'] = $tmp['video_mobile_path'] ? $video_server.$tmp['video_mobile_path'] : $video_server.$tmp['video_path'];
                } else {
                    $res_val['flashimg'] = UPLOAD_URL.'/'.$tmp['flashimg'];
                    $pos = stripos($tmp['body'], 'http');
                    $res_val['flashvar'] = substr($tmp['body'], $pos);
                }
                $photo_video_list[] = $res_val;
            }else{
                $map = array();
                $map['row_id'] = $value['feed_id'];
                $map['is_del'] = 0;
                $attachList = model('Attach')->field('attach_id,save_path,save_name,row_id')->where($map)->order('attach_id Desc')->findAll();
                $attach_val = array();
                foreach ($attachList as $k1 => $val) {
                    $attach_val['feed_id'] = $value['feed_id'];
                    $attach_val['type'] = $value['type'];
                    $attach_val['image_id'] = $val['attach_id'];
                    $attach_val['image_url'] = getImageUrl($val['save_path'].$val['save_name']);
                    $photo_video_list[] = $attach_val;
                }
            }

        }

        return $photo_video_list;
    }
    ...
    public function save_user_info()
    {
        ...
        //修改匿名名称
        if(isset($this->data['anonymous_change'])){
            $anonymous_info = $this->create_user_anonymous($this->mid);
            //清除用户缓存
            model('User')->cleanCache($this->mid);
            $return_map = array();
            $return_map['anonymous_name'] = $anonymous_info['nickname'];
            $pic = D('attach')->where('attach_id='.$anonymous_info['icon'])->find();
            $pic_url = $pic['save_path'].$pic['save_name'];
            $return_map['anonymous_icon'] = getImageUrl($pic_url);
            $return_map['anonymous_bd_time'] = $anonymous_info['bd_time'];
            $return_map['anonymous_change_title'] = '每两天可以更新一次花名';
            $return_map['anonymous_change'] = '0';
            $return_map['status'] = 1;
            return $return_map;
        }
     ...
     * 用户匿名名称生成
     */
    public function create_user_anonymous($uid=''){
        if($uid){
            $c_uid = $uid;
        }else{
            $c_uid = $this->mid;
        ...
        $map = array();
        $map['a.use_count'] = 0;
        $map['a.pid'] = array('neq','0');
        $tablePrefix = C('DB_PREFIX');
        $anonymous = D('anonymous')
            ->field('a.id,a.use_count,a.all_count,a.nickname,b.icon')
            ->where($map)
            ->table("{$tablePrefix}anonymous AS a LEFT JOIN {$tablePrefix}anonymous AS b ON b.id=a.pid")
            ->order('b.weight DESC,RAND()')->find();
        if($anonymous){
            $anonymous_id = $anonymous['id'];
            $use_count = $anonymous['use_count'];
            $all_count = $anonymous['all_count'];
        ...
        //查询原匿名名称
        $old_anonymous = D('user_anonymous')->where('`uid`='.$c_uid)->find();
        if($old_anonymous){
            $map = array();
            $map['id'] = $old_anonymous['anonymous_id'];
            $map['use_count'] = array('exp','use_count-1');
            $save_res = D('anonymous')->save($map);
            if(!($save_res===false)){
                //用户设置新匿名名称
                $map = array();
                $map['anonymous_id'] = $anonymous_id;
                $map['bd_time'] = time();
                D('user_anonymous')->where('`uid`='.$c_uid)->save($map);
                //更新新匿名的使用次数
                $map = array();
                $map['id'] = $anonymous_id;
                $map['use_count'] = $use_count + 1;
                $map['all_count'] = $all_count + 1;
                D('anonymous')->save($map);
            }
        }else{
            //添加新匿名
            $map = array();
            ...
            $map['anonymous_id'] = $anonymous_id;
            $map['bd_time'] = time();
            $add_res = D('user_anonymous')->add($map);
            if($add_res){
                //当前匿名使用人数和总人数加1
                $map = array();
                $map['id'] = $anonymous_id;
        ...
        model('Cache')->rm('user_info_api_'.$c_uid);
        $return_map = array();
        $return_map['nickname'] = $nickname;
        $return_map['icon'] = $icon;
        $return_map['bd_time'] = time();
        return $return_map;
    }
    ...
    {
        $limit = $this->data['limit'] ? $this->data['limit'] : 10;
        $list = model('UserStar')->getStarUserForApi($limit);
        //查询当前用户人气排行
        $num = '1';
        $uid = $this->mid;
        $my_order = '未上榜';
        ...
        $list['my_name'] = $user_info['uname'];
        $list['my_avatar'] = $user_info['avatar_big'];
        return $list;
    }
