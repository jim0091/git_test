...
class EventApi extends Api
{
     ...
     * @author Seven Du <lovevipdsw@vip.qq.com>
     **/
    public function getInfo($eid = 0)
    {
        $id = empty($eid) ? Common::getInput('eid') : $eid;
        ...
        $data['sexcount'] = array('boy'=>0,'girl'=>0);
        foreach($data['enrollmentUsers'] as $v){
            if($v['sex'] == 1){
                $data['sexcount']['boy']++;
            }else{
                $data['sexcount']['girl']++;
        ...
        $data['is_favorite'] = model('Collection')->isCollection($id, 'event_list');
     ...
     *
     * @param
     *          integer event_id 资源ID
     *
     * @return array 状态+提示
     */

    public function favorite_event()
    {
        $data['source_table_name'] = 'event_list'; // event_list
        $data['source_id'] = $this->data['event_id']; // 140
        $data['source_app'] = 'event'; // event
     ...
     * @return array 状态+提示
     */
    public function unfavorite_event()
    {
        if (model('Collection')->delCollection($this->data['event_id'], 'event_list')) {
            return array(
                'status' => 1,
                'msg'    => '取消收藏成功',
            );
     ...
     * @return array 状态+提示
     */
    public function getfavorite_event()
    {
        $limit = empty($this->data['count']) ? 10 : $this->data['count'];
        $uid = empty($this->data['uid']) ? $GLOBALS['ts']['mid'] : $this->data['uid'];
        $map = array(
            'source_app' => 'event',
            'source_table_name' => 'event_list',
            'uid' => $uid,
        );
        $list = model('Collection')->getCollectionList($map,$limit);
        $re = array();
        foreach($list['data'] as $key => $val){
            if (!($data = Event::getInstance()->get($val['source_id'])) or $data['del']) {
               continue;
            }else{
                $eventlist = $this->getInfo($val['source_id']);
                $eventlist['eid'] && $re[] = $eventlist;
                unset($eventlist);
            }
        }
        return $re;
