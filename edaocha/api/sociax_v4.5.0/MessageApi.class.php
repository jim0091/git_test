...
'comment' => 0, 'atme' => 0, 'digg' => 0, 'follower' => 0, 'weiba' => 0, 'weiba_comment' => 0, 'unread_digg_weibapost' => 0,
        ...
        return array(
            ...
            'anonymous_comment'               => (string) intval($count['anonymous_unread_comment']),
            'anonymous_digg'                  => (string) intval($count['anonymous_unread_digg']),
        );
    }
    ...
    public function set_disturb()
    {
        $room_id = $this->data['room_id'] ?  intval($this->data['room_id']) :  0 ;
        $disturb = $this->data['setdisturb'] ?  intval($this->data['setdisturb']) :  0 ;

        empty($room_id) && $this->error(array(
            'status' => 0,
            'msg'    => '请选择群聊ID',
        ));
        ...
        if(D('message_member')->where($map)->save(array('is_disturb' => $disturb))){
            return array(
                'status' => 1,
                'msg'    => '设置成功',
            );
