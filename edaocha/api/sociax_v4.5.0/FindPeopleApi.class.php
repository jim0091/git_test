...
class FindPeopleApi extends Api
{
    ...
    public function search_user()
    {
        ...
        if ($key) {
            ...
            //搜索统计
            $s_count = $user_list?count($user_list):0;
            model('KeywordStatistic')->setKeywordStatistic($key,2,$s_count);
        } else { // 获取感兴趣的5个人
            ...
            //获取管理员推荐的20个用户
            $user_list = model('Cache')->get('related_user_new'.$GLOBALS['ts']['mid']);
            if (empty($user_list)) {
                $map = array();
                $map['uid'] = array('in','17709,18024,17877,17510,17506,17875,17717,18026,17626,17879,17858,17637,17878,18005,65,17681,17617,17720,17819,146');
                $user_list = model('User')->where($map)->field('uid,uname,intro')->order('uid desc')->findAll();
                foreach ($user_list as $k => $v) {
                    $user_list[$k]['uid'] = $v['uid'];
                    $user_list[$k]['uname'] = $v['uname'];
                    $user_list[$k]['remark'] = D('UserRemark')->getRemark($this->mid, $v['uid']);
                ...
                model('Cache')->set('related_user_new'.$GLOBALS['ts']['mid'], $user_list);
            }
