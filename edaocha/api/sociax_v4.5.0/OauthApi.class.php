...
class OauthApi extends Api
{
    ...
    public function signIn()
    {
        ...
        $register_code = t($this->data['register_code']); // 注册邀请码判断
        if($register_code){
            //判断邀请码的有效性和正确性
            $map = array();
...
// 				$return = array('status'=>1, 'msg'=>'注册成功', 'need_audit'=>0);
    ...
    public function bind_new_user()
    {
        ...
        //邀请码验证
        $register_code = t($this->data['register_code']);
        if($register_code){
            //判断邀请码的有效性和正确性
            $map = array();
            $map['code'] = $register_code;
        ...
        if ($uid) {
            ...
            if($codeInfo['id']){
                $code_map = array();
                $code_map['code_id'] = $codeInfo['id'];
                $code_map['uid'] = $uid;
                $code_map['bind_time'] = time();
                model('RegisterCode')->codeBind($code_map);
            }
    ...
    public function get_version(){
        $type = intval($_REQUEST['type']); // 0-安卓  1-IOS
        if($type){
            $type = 1;
        }else{
            $type = 0;
        ...
        $map['type'] = $type;
        $map['is_old'] = 0;
        $map['version'] = t($_REQUEST['version']);
        $versions = D('version')->field('version,version_img,version_title')->where($map)->find();
        if($versions['version']){
            //传过来的是最新版本
            return array(
                'status' => 0,
            ...
            $map['type'] = $type;
            $map['is_old'] = 0;
            $version_info = D('version')->field('version,version_img,version_title')->where($map)->find();
            $request_version = str_replace('.', '', t($_REQUEST['version']));
            $server_version = str_replace('.', '', $version_info['version']);
            if($request_version>$server_version){
                ...
                $map['is_old'] = 1;
                $map['version'] = t($_REQUEST['version']);
                $old_info = D('version')->where($map)->find();
                if($old_info){
                    if ($old_info['update_type']) {
                        $msg = '强制更新';
                        $status = 2;
                    }else{
                        $msg = '更新';
                    ...
                    if($type){
                        ...
                        $url = "https://v1.edaocha.net/download/package/aodou_".$version_info['version'].".apk";
                    }
                    return array(
                        'status' => $status,
                        'msg'    => $msg,
                        ...
                        'version_img' =>"https://v2.edaocha.net/".$version_info['version_img'],
                    );
                }else{
                    return array(
                        'status' => 0,
