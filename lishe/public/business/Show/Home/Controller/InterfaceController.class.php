...
class InterfaceController extends CommonController {
	...
	public function updateJdSaleStatus(){
		...
		$isForce = I('post.isForce'); //1：正常 2：强制
		if(empty($isForce)){
			$isForce=1;
		}
		if(!empty($sku) && in_array($status,array(0,100))){
			$condition['jd_sku']=$sku;
			$item=$this->modelItem->field('item_id,shop_id,price,cost_price')->where($condition)->find();
		...
		$rate=($item['price']-$item['cost_price'])*100/$item['price'];
		if($rate<C('JD_PROFIT_RATE')){
			$status=0;
		}
		$statusArr=array(
			0=>'instock',
		...
		$ret= $this->updateItemStatus($item['item_id'],$item['shop_id'],$statusArr[$status],$isForce);
	...
	public function updateItemStatus($itemId,$shopId,$status,$isForce=1){
		...
		$item=$this->modelItemStatus->field('item_id,is_force')->where($condition)->find();
		if(empty($item['item_id'])){
			$data['item_id']=$itemId;
			return $this->modelItemStatus->add($data);
		}else{
			//if($item['is_force']>$isForce && $status=='onsale'){
			if($item['is_force'] == 3 && $status=='onsale'){
				return NULL;
			}
		    	...
		    	if($rets['code']==100){
		    		if($rets['errCode']==0){
			    		...
			    		$payData=array(
			    			...
			    			'ip'=>$ip,
			    			'memo'=>$returns,
			    			'return_url'=>serialize(array('topc_ctl_paycenter@finish',array('payment_id'=>$paymentId))),
			    			'trade_no'=>$paymentId,
			    			'tids'=>$tids
			    		);
