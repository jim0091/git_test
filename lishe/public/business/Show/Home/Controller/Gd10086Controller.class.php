...
/**
  +------------------------------------------------------------------------------
  +------------------------------------------------------------------------------
 * @version  	$Id: Gd10086Controller.class.php v001 2016-8-24
 * @description 广东10086
  +------------------------------------------------------------------------------
 */
...
class Gd10086Controller extends CommonController {
	public function __construct(){
		...
		$this->gd10086ComId='1467166836740';
		...
		if(strtolower(ACTION_NAME)!='login' && strtolower(ACTION_NAME)!='activate'){
			if(!empty($this->comId) && $this->comId!=$this->gd10086ComId){
			...
			if(!empty($menusConfig)){
				$catConfigArr=$this->modelCatConfig->field('cat_config_id,cat_id,cat_name,item_config_id')->where('disabled=0 AND com_id='.$this->gd10086ComId)->order('order_sort DESC')->select();				
	...
	public function index(){
		header("Location:http://gd10086.lishe.cn/b.php/Gd10086");
		exit;
		//查询可用余额  begin
		...
		$sign=md5('appKey='.$appKey.'&mobileNo='.$this->account.C('API_AOSERVER_KEY'));
		$data=array(
    		'appKey'=>$appKey,
    	...
    	$sign=md5('appKey='.$appKey.'&empCode='.$empCode.C('API_AOSERVER_KEY'));
		$datas=array(
    		'appKey'=>$appKey,
    		'empCode'=>$empCode,
    		'sign'=>$sign
    	);  
		$res=$this->accreditPost($url,json_encode($datas),$user,$password);
		$personInfo=json_decode($res,TRUE);
		$balance=$personInfo['data']['posBalance'];
...
//取出一级分类下的二级分类
			...
			foreach($category as $key => $value){
				if(empty($value['item_ids'])){											
					...
					if(!empty($categorysId[$key])){
						$condition='i.item_id<='.$this->modelItemId.' AND i.flag=0 AND i.shop_id=10 AND i.profit_rate>='.$value['profit_rate'].' AND i.cat_id IN('.implode(',',$categorysId[$key]).') AND i.item_id=is.item_id AND is.approve_status=\'onsale\'';
					}else{
						$condition = ' 1 > 1';
					}
				}else{
		...
		if(!empty($catId) && intval($catId)>0){
			...
			if(!empty($category['cat_id'])){
				...
				if(!empty($categorysArr)){
					...
					$condition='i.item_id<='.$this->modelItemId.' AND i.shop_id=10 AND i.profit_rate>='.$category['profit_rate'].' AND i.cat_id IN ('.implode(',',$catIds).') AND i.item_id=is.item_id AND is.approve_status=\'onsale\''.$where;
	...
	public function activate(){
    	...
    	if($ret['code']==100){
	    		//注册用户
    		unset($data);
    		$sign=md5('com_id='.$this->gd10086ComId.'&login_pwd='.$password.'&phone_num='.$userName.C('API_KEY'));
				$data=array(
					'com_id'=>$this->gd10086ComId,
					'login_pwd'=>$password,
		        	'phone_num'=>$userName,
		        	'sign'=>$sign
		        );
        ...
        if($return['result']==100){
        	...
        	$balance=array(
        		...
        		'commonAmount'=>0,
        		'limitAmount'=>0,
        		'comId'=>$this->gd10086ComId,
        		'comName'=>$data['comName']
        	);
        	$userId=A('User')->register($user,$info,$balance);
		        	
					//查询mark
					$condition['com_id']=$this->gd10086ComId;
	...
	public function login(){
		if(!empty($this->uid)){
			header("Location:".__APP__."/Gd10086/index");
		}
		if($_GET['go']=='cart'){
			...
			$this->assign('refer',__APP__.'/Gd10086');
		}		
		$this->display();
	}
