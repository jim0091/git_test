...
class CartModel extends Model{
	...
	public function getCartList($condition){
		if (empty($condition)) {
			...
			return $this->modelCart->table('systrade_cart c,sysitem_sku s')->where('c.sku_id = s.sku_id '.$condition)->field('c.cart_id,c.shop_id,c.item_id,c.sku_id,c.title,c.image_default_id,c.quantity,s.price,s.cash,s.type,s.point,s.spec_info,s.weight,s.activity_config_id,s.aitem_id,s.disable')->select();
		}
	}
	//根据itemIds查询商品上下架状态
	...
	public function getDefaultAddressInfo($condition){
		if (!$condition) {
			...
			return $this->modelAddr->where($condition)->find();
		}		
	}
	...
	//购物车减少商品
	public function decCartNum($condition){
		if (!$condition) {
			return false;
		}else{
			return $this->modelCart->where($condition)->setDec('quantity',1);
		}
	}
	//查询购物车单个商品信息
