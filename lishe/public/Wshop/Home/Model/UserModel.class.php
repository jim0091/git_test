...
class UserModel extends Model{
	public function __construct(){		
        ...
        $this->modelPayment = M('ectools_payments');//支付表
	}
	...
	public function getOrderList($condition,$limit){
		if (!$condition) {
			...
			return $this->modelTrade->where($condition)->field('tid,shop_id,status,order_status,buyer_area,post_fee,payment,refund_fee')->order('created_time desc')->limit($limit)->select();
		}
	}
	//获取用户订单数量
	public function getOrderCount($condition){
		if (!$condition) {
			...
			return $this->modelTrade->where($condition)->count('tid');
		}
	}
	//根据店铺ids查询店铺信息
