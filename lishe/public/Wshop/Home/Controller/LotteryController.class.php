...
class LotteryController extends CommonController {
	...
	public function index(){
		...
		$lotteryUserList = M('lottery_user')->field('username,prize_name')
								...
								->select();
		//填充临时数据
		$lotteryUserList = $this->fillLottery($lotteryUserList);
...
// 	}
	...
	/**
	 * 填充临时数据
	 * @param unknown $lotteryUserList
	 * @return string[][]
	 */
	private function fillLottery($lotteryUserList){
		$tmpUserList = array();
		$tmpUserList[] = array(
				'username' =>'章*',
				'prize_name' => '100积分',
		...
		if(empty($lotteryUserList) || !is_array($lotteryUserList)){
			return $tmpUserList;
		}else{
			return array_merge($lotteryUserList, $tmpUserList);
		}
