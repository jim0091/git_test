...
class ItemsController extends CommonController{
	...
	public function condition(){
		...
		$keyword=str_replace(array("'"),array(""),$keyword);//清除'，防止报错
		//$keywordArr = explode(' ',$keyword);

		// if (is_array($keywordArr) && $keywordArr) {
		// 	$con = '';
		// 	foreach ($keywordArr as $key => $value) {
		// 		$con .= " title like '%".$value."%' or keywords like '%".$value."%' or";
		// 	}

		// 	$condition .=" AND (".rtrim($con,'or').")";
		// }
		if ($keyword) {
