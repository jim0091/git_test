...
class PayController extends CommonController {
  ...
  public function __construct(){
    ...
    public function addUserOrder(){
        ...
        $postData['orderFrom'] = "pc";//pc：心意商城，show：企业秀，app：手机app，ws微信商城
    ...
    public function activityPay(){
        $paymentid = I("get.paymentid");
				//取出活动的标识
				$identification=$this->dActivity->getActivityIdendity($paymentid);
				if($identification=="christmasApple"){
					//校验资格
					$checkRes=A('Promotion')->checkBuyApple();
					if($checkRes['code']==0){
						//不满足资格
    ...
    public function operPayActivity(){
        ...
        $payName = I('post.payName');
				...
				if($identification=="christmasApple"){
					...
					if($checkRes['code']==0){
						//不满足资格
            echo json_encode(array(0,$checkRes['msg']));
            exit();	
					}			
				}	
        if (empty($paymentid)) {
