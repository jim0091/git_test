...
class GoodscatModel extends Model{
	...
	**/
	public function randCatItem($catId,$level){
		$where = 'a.item_id=b.item_id and a.item_id = s.item_id and b.approve_status="onsale" and a.disabled=0 and s.store > 0';
		if($level == 1){
		...
		return $this->dbItem
					->table('sysitem_item a,sysitem_item_status b,sysitem_item_store s')
                    ->where($where)
                  	->field('a.item_id,a.title,a.image_default_id,a.price')
					->limit(6)
					->order('rand()')
					->select();
	}
