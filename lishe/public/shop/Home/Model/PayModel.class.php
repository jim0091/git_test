...
class PayModel extends Model{
	...
	public function getTradeList($condition){
		if (!$condition) {
			...
			return $this->modelTrade->where($condition)->field('shop_id,created_time')->select();
		}		
	}
	//根据tid查询订单子表
	public function getOrderList($condition){
		if (!$condition) {
			...
			if (!$orderList) {
				...
				$itemSkuList = $this->modelSku->where('sku_id in('.implode(',',$skuIds).')')->field('sku_id,item_id,price,cost_price,type')->select();
				foreach ($orderList as $key => $value) {
