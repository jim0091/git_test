...
/**
  +------------------------------------------------------------------------------
  +------------------------------------------------------------------------------
 * @version  	$Id: CsgController.class.php v001 2016-8-24
 * @description 南方电网
  +------------------------------------------------------------------------------
 */
namespace Home\Controller;
class CsgController extends CommonController {
	public function __construct(){
		...
		if(strtolower(ACTION_NAME)!='login' && strtolower(ACTION_NAME)!='activate'){
			...
			if(!empty($menusConfig)){
				$catConfigArr=$this->modelCatConfig->field('cat_config_id,cat_id,cat_name,item_config_id')->where('disabled=0 AND com_id='.$this->csgComId)->order('order_sort DESC')->select();				
...
//取出一级分类下的二级分类
	...
	public function login(){
		if(!empty($this->uid)){
			header("Location:".__APP__."/Csg/index");
		}
		if($_GET['go']=='cart'){
	...
	public function logout(){
		session(null);
		cookie('account',null);
		header("Location:".__APP__."/Csg/login");
	}
