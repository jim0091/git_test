...
class Zs10086Controller extends CommonController {
	public function __construct(){
		...
		if(strtolower(ACTION_NAME)!='login' && strtolower(ACTION_NAME)!='activate'){
			...
			if(!empty($menusConfig)){
				$catConfigArr=$this->modelCatConfig->field('cat_config_id,cat_id,cat_name,item_config_id')->where('disabled=0 AND com_id='.$this->zs10086ComId)->order('order_sort DESC')->select();				
	...
	public function getCategory($cfgId=0){
		...
		$catConfig=array();
		$catConfigArr=$this->modelCatConfig->field('item_config_id,cat_config_id,recommend,cat_id,cat_ids,cat_name,profit_rate,item_ids,shop_id')->where($condition)->order('order_sort DESC')->select();
...
//取出一级分类下的二级分类
	...
	public function activate(){
    	...
    	if($ret['code']==100){
	    		//注册用户
    		unset($data);
    		$sign=md5('com_id='.$this->zs10086ComId.'&login_pwd='.$password.'&phone_num='.$userName.C('API_KEY'));
				$data=array(
					'com_id'=>$this->zs10086ComId,
					'login_pwd'=>$password,
		        	'phone_num'=>$userName,
		        	'sign'=>$sign
		        );
        ...
        if($return['result']==100){
        	...
        	$balance=array(
        		...
        		'commonAmount'=>0,
        		'limitAmount'=>0,
        		'comId'=>$this->zs10086ComId,
        		'comName'=>$data['comName']
        	);
        	$userId=A('User')->register($user,$info,$balance);
		        	
					//查询mark
					$condition['com_id']=$this->zs10086ComId;
	...
	public function login(){
		if(!empty($this->uid)){
			header("Location:".__APP__."/Zs10086/index");
		}
		if($_GET['go']=='cart'){
			...
			$this->assign('refer',__APP__.'/Zs10086');
		}
		$this->assign('refer','http://zs10086.lishe.cn/b.php/Zs10086');	
		$this->display();
	}
	...
	public function logout(){
		...
		cookie('LSUID',null);
		cookie('UNAME',null);
		header("Location:".__APP__."/Zs10086/login");
	}
