...
//默认加载库存表价格
var defPrice = $("#size div").children("input.skuPrice").val();
var defShopPrice = $("#size div").children("input.skuShopPrice").val();
var defIntegral = Math.round(defPrice*100);
if(defShopPrice > defPrice){
	var defShopIntegral = Math.round(defShopPrice*100);
	$(".itemShopBalance").text(defShopIntegral);
	$(".itemShopPrice").text("￥"+parseFloat(defShopPrice).toFixed(2));	
}
$(".itemBalance").text(defIntegral);
$(".itemPrice").text("￥"+parseFloat(defPrice).toFixed(2));
...
function getStockOpt(id,name){
	...
	var address = currentAreaInfo.currentProvinceId+'_'+currentAreaInfo.currentCityId+'_'+currentAreaInfo.currentAreaId+'_'+currentAreaInfo.currentTownId;    
...
function checkStore(){
	...
	if (shopId != 10) {
	    ...
	    $.get("http://www.lishe.cn/api.php/Jd/checkJdStock/item_id/"+item_id+"/jd_ids/"+jd_ids+"/num/"+number.value, function(retData){
	    	if(retData==33){
				var msg='有货';
				$("#size div").bind("click");
				$('#add_to_cart').show();
				...
				var msg='无货';
				$("#size div").unbind("click");
				$('#add_to_cart').hide();
				$('#outofstock').show();
			}
			$('.lishe_goods_for').text(msg);
		});
	}
}
...
function checkedInven(){
    ...
    if(skuType == 1){
    	...
    	skuNum = $("#sku-info-s-"+acid).attr('skuNum');
    }else if (skuType == 4) {
    	skuNum = $(".groupBuy.active").eq(0).attr('skuNum');
    }else {
    	$("#number input").val(1);
    	var skuId = $("#selectedSku").val();
    	skuNum = $("#sku_"+skuId).find('.skuNum').val();
    }
	if (skuNum <= 0) {
		$('#add_to_cart').hide();
		$("#outofstock").show();
...
function CurrentAreaInfoInit(){
	...
	currentAreaInfo =  {"currentLevel": 1,"currentProvinceId": 19,"currentProvinceName":"广东","currentCityId": 0,"currentCityName":"","currentAreaId": 0,"currentAreaName":"","currentTownId":0,"currentTownName":""};  
	var ipLoc = getCookie("ipLoc-djd");
	// ipLoc = ipLoc?ipLoc.split("-"):[1,72,0,0];
	ipLoc = ipLoc?ipLoc.split("-"):[19,1607,3155,0];
...
var page_load = true;
(function(){
	$("#store-selector").unbind("click").bind("mouseover",function(){
	...
	$("#size div").click(function(){
		...
		//调整时间
		$(".sku-info-box").hide();
		var skuTypeFlag = 'nomal';
		var acName = "";
		if(skuType == 1){
			skuTypeFlag = "s"
			acName = "礼舍秒杀";
			$("#acIcon").addClass("tlLookIcon2");
		}else if(skuType == 4){
			skuTypeFlag = "g" 
			acName = "限时拼团";
			$("#acIcon").removeClass("tlLookIcon2");
		}
		$("#sku-info-"+skuTypeFlag+'-'+acid).show();
		if(skuType == 4){
			$("#sku-info-"+skuTypeFlag+'-'+acid+" .select_buy_content_ul li:first").click();
		}
		if (acid != 0) {
			$("#acName").text(acName);
			$("#time-limit-box").show();
		...
		$("#selectedSku").val(skuId);
		var athis = $(this);
		var skuPrice = athis.children("input.skuPrice").val();
		var skuShopPrice = athis.children("input.skuShopPrice").val();
		var newSkuShopPrice = parseFloat(skuShopPrice).toFixed(2);
		var newSkuPrice = parseFloat(skuPrice).toFixed(2);
		var integral = Math.round(newSkuPrice*100);
		if(newSkuShopPrice > newSkuPrice){
			var oldIntegral = Math.round(newSkuShopPrice*100);
			$(".itemShopPrice").text("￥"+newSkuShopPrice);
			$(".itemShopBalance").text(oldIntegral);
		}
		//$(".itemBalance").text(integral);
		$(".itemPrice").text("￥"+newSkuPrice);

		//加载售价和积分价
		var skuCash = athis.children("input.skuCash").val();
		var skuPoint = athis.children("input.skuPoint").val();
		//console.log(skuCash);
		//console.log(skuPoint);
		if (skuCash == '0.000' && skuPoint == '0' ) {
			$(".tdTitle").text("福利积分：");
			$(".skuCashPoint").text(integral);
			$(".jifen").text("积分");
		}else if(skuCash == '0.000' && skuPoint != '0'){
			$(".tdTitle").html("积&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;分：");
			$(".skuCashPoint").text(skuPoint);
			$(".jifen").text("积分");
		}else if(skuPoint == '0' && skuCash != '0.000'){
			$(".tdTitle").html("现&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;金：");
			$(".skuCashPoint").text("￥"+parseFloat(skuCash).toFixed(2));
			$(".jifen").text("");
		}else{
			$(".tdTitle").text("现金+积分：");
			$(".skuCashPoint").text("￥"+parseFloat(skuCash).toFixed(2)+"+"+skuPoint);
			$(".jifen").text("积分");
		}
		checkedInven();			
	});	
