...
class OrderModel extends Model{
	...
	public function getCartList($condition){
		if (empty($condition)) {
			...
			return $this->modelCart->table('systrade_cart c,sysitem_sku s')->where('c.sku_id = s.sku_id and s.disable = 0 '.$condition)->field('c.cart_id,c.shop_id,c.item_id,c.sku_id,c.title,c.image_default_id,c.quantity,s.price,s.spec_info,s.weight,s.cash,s.point,s.type,s.activity_config_id,s.aitem_id')->select();
		}
	}
	//根据itemIds查询商品上下架状态
