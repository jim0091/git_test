...
class InfoController extends CommonController {
    ...
    //产品利润率计算方法return 33有货，0无货
    public function itemGrossMargin($itemId){
        if (empty($itemId)) {
            return 0;
        }
        //查询商品
        $condition = array('item_id'=>$itemId);
        $field = array('price,cost_price');
        $itemInfo = $this->modelGoodsCat->getItemInfo($condition,$field);
        if (!$itemInfo) {
            return 0;
        }
    ...
    public function index(){
        ...
        if (!empty($itemId)){
            $fromMd5 = md5($itemId);
            if ($fromMd5 != $from) {
                $store = $this->itemGrossMargin($itemId);
            }  
        	//得到企业销售规则
        	$param=array(
        		'type'=>'itemLimit',
            ...
            if(!empty($skuList)){
            	$skuStore = $this->itemSkuStoreModel->where('item_id = '.$itemId)->select();
				foreach($skuStore as $key=>$value){
					$remain[$value['sku_id']]=$value['store']-$value['freez'];
				}
				foreach($skuList as $key=>$value){
					if($key==0){
						$selectedSku=$value['sku_id'];
					}
					$sku[$value['sku_id']]=$value;
					$sku[$value['sku_id']]['remain']=$remain[$value['sku_id']];
				}
			}        
            //更多精选商品
            //产品利润率
    ...
    public function addItemCart(){        
        ...
        //积分限制加入购物车
        $ItemFrom = $this->getComDomain();
        $map = array(
            'user_id' => $this->uid,
            'pointsChannel' => $ItemFrom
        );
        $canJoin = M('sysuser_user_deposit_detail')->where($map)->getField('point_detail_id');
        if(!$canJoin){
            $this->ajaxReturn(array(0,'无该类型积分，加入购物车失败！',''));
        }
        //团购秒杀购买次数限制
