...
class syslogistics_api_dlytmpl_delete{
    ...
    public function delete($params)
    {
        ...
        $filterdata['dlytmpl_id'] = $params['template_id'];
        $filterdata['shop_id'] = $params['shop_id'];
        $filterdata['status'] = 'WAIT_SELLER_SEND_GOODS,WAIT_BUYER_PAY';
        $filterdata['fields'] = 'tid,status';
        $tradeList = app::get('topshop')->rpcCall('trade.get.list',$filterdata,'seller');
        if($tradeList['count']>=1)
        {
             throw new \LogicException('该快递模板下面还有未发货的订单！');
        }
