...
class sysaftersales_ctl_refunds extends desktop_controller {
    ...
    public function refundsPay($refunds_id)
    {
        ...
        // 获取关联的一企一舍用户ID
        $ls_user_id = app::get('sysuser')->model('user')->getRow('ls_user_id', ['user_id'=>$data['user_id']]);
        $ls_user_id = $ls_user_id['ls_user_id'];

        $data['ls_user_id'] = $ls_user_id;
    ...
    public function dorefund()
    {
        ...
        try
        {
            ...
            if($refunds['status']==1)
            {
                throw new \LogicException(app::get('sysaftersales')->_('订单已处理，请勿重复操作'));
            }
			...
			//调用接口一企一舍退款 赵尊杰 2016-06-03
			$transno=app::get('systrade')->database()->executeQuery('SELECT transno FROM systrade_trade where tid='.$refunds['tid'])->fetch();
			$paymentId=app::get('ectools')->database()->executeQuery('SELECT payment_id FROM ectools_trade_paybill where status=\'succ\' AND tid='.$refunds['tid'])->fetch();
			$url = config::get('link.lishe_company_url').config::get('link.company_refundOrder');
			$refundFee=$refunds['refund_fee']*100;
	        $sign=md5('orderno='.$paymentId['payment_id'].'&refundAmount='.$refundFee.'&transno='.$transno['transno'].config::get('link.company_key'));
			$data=array(
	        	'orderno'=>$paymentId['payment_id'],
            ...
            if($refundsData['rufund_type'] == 'deposit')
            {
                ...
                $params['ls_user_id']  = $refundsData['ls_user_id'];
