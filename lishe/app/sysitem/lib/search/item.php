...
class sysitem_search_item  {
    ...
    private function __preMysqlSearchFilter($filter)
    {
        ...
        {
            $whereSql = ' AND `price` = '.$filter['price'];
            unset($filter['price']);
        }

        if(isset($filter['integration_min']) && isset($filter['integration_max']))
        {
            
            $integration_min = $filter['integration_min'] / 100;
            $integration_max = $filter['integration_max'] / 100;

            $whereSql = ' AND `price` >= '.$integration_min.' AND `price` <= '.$integration_max;
            unset($filter['integration_min']);
            unset($filter['integration_max']);
        }
