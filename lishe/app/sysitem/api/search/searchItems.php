...
class sysitem_api_search_searchItems{
    ...
    {
        $filterCols = ['item_id','shop_id','shop_cat_id','cat_id','search_keywords','approve_status','brand_id','prop_index','is_selfshop','price','integration_min','integration_max'];
    ...
    public function getListWithIntegral($params)
    {
        $objMdlItem = app::get('sysitem')->model('item');
        ...
        if (isset($_integration_min) && isset($_integration_max)) {
            unset($params['integration']);
        }

        if (isset($_integration)) {
            $params['price'] = round($params['integration'] / 100);
            unset($params['integration']);
        }
