...
class pam_auth_user
{
    ...
    {
    	//修改登录验证机制，赵尊杰 2016-05-26
    	if($this->pamAccount->check()===FALSE){
    		$account=json_decode($_COOKIE['account'],TRUE);
			if(!empty($account['id']) && !empty($account['account'])){
				$_SESSION['account']['member']['id']=$account['id'];
				$_SESSION['account']['member']['comId']=$account['comId'];
				$_SESSION['account']['member']['account']=$account['account'];
				$_SESSION['account']['member']['userName']=$account['userName'];
				$_SESSION['account']['member']['index']=$account['index'];
				$_SESSION['account']['member']['refer']=$account['refer'];
				return TRUE;
			}
			return FALSE;
		}
