...
class topc_ctl_member extends topc_controller {
    ...
    public function index()
    {
        ...
        header('Location: http://www.lishe.cn/user.php/Order/orderList');
        //会员信息
        ...
        $pagedata['trades'] = $tradelist['list'];
         ...
         foreach($pagedata['trades'] as $key=>$val)
        {
            $pagedata['trades'][$val['tid']]['payment_deposit'] = $pagedata['trades'][$val['tid']]['payment'] * 100;
        }
        ...
        /*if ($pagedata['deposit']) {
            ...
            $tmp_deposit = $pagedata['deposit']['deposit']['deposit'];
            
            if (isset($tmp_deposit)) {
                $pagedata['deposit']['deposit'] = $tmp_deposit * 100;
                $pagedata['deposit']['price'] = $tmp_deposit;
            }
        }*/
...
//echo '<pre>';print_r($pagedata);exit();
    ...
    public function address()
    {
        ...
        $pagedata['refer'] = trim($_GET['refer']);
    ...
    public function saveAddress()
    {
        ...
        unset($postData['refer']);
        if(empty($postData['addr_id'])){
    ...
    public function saveModifyPwd()
    {
        try{
            ...
            $validator = validator::make(
                ...
                ['oldpassword' => '原密码不能为空！' ,'password' => '密码长度不能小于6位!|密码长度不能大于20位!|输入的密码不一致!','password_confirmation' =>'确认密码不能为空!']
            );
            if ($validator->fails())
            {
                $messages = $validator->messagesInfo();
            ...
            $account=userAuth::getLoginName();            
            //调用接口更改密码
	        $url = config::get('link.lishe_company_url').config::get('link.company_updatePass');
	        $sign=md5('doType=fg&newPass='.$postData['new_password'].'&phoneNum='.$account.config::get('link.company_key'));
			$data=array(
	        	'doType'=>'fg',
			...
			if($reMsg['result']==100){
				if($reMsg['errcode']>0){
					return $this->splash('error','',$reMsg['msg'],true);
				}				
			}else{
				return $this->splash('error','',$reMsg['msg'],true);
			}
			$url = url::action("topc_ctl_member@security");
        	$msg = app::get('topc')->_('密码修改成功');
        	return $this->splash('success',$url,$msg);
        }
        catch(Exception $e)
        {
    ...
    public function verify()
    {
        ...
        $pagedata['type']= in_array($postData['type'],$this->sendType)?$postData['type']:$this->sendType[0];
