...
class topc_ctl_item extends topc_controller {
    ...
    public function index()
    {
        ...
        header('Location: http://www.lishe.cn/shop.php/Info/index/itemId/'.$itemId.$from);
        ...
        $url='/shop.php/Info/index/itemId/'.$itemId;
        if(!empty($_GET['from'])){
			$url.='/from/'.$_GET['from'];
		}
        ...
        if($detailData['use_platform'] == 2 ){
        	header("Location:/business/wshop.php/Info/index/itemId/".$itemId);exit;
        }
        ...
        if($activityDetail)
        {
            $activityDetail['activity_deposit'] = round($activityDetail['activity_price'] * 100);
        ...
        // add by yuan 201604141735
        $detailData['price_deposit'] = round($detailData['price'] * 100);        
        ...
        setcookie('browList',json_encode($browListArr),time()+3600*24,'/','.lishe.cn');
        
		$account=json_decode($_COOKIE['account'],TRUE);
        if($_GET['from']!=md5($itemId) && $account['comId']=='1467166836740'){
			$pagedata['item']['realStore']=0;
		}
    ...
    private function __searchRate($itemId)
    {
        ...
        $filter = input::get();
        $filter['pages'] = time();
        if($data['total_results']>0) $total = round($data['total_results']/10);
