...
class topc_ctl_shopcenter extends topc_controller
{
    ...
    public function index()
    {
        ...
        if( $items )
        {
            ...
            foreach( $items as &$row )
            {
                foreach( $row as &$item )
                {
                    $item['price_deposit'] = $item['price'] * 100;
                    // var_dump($item);
                }
            }
            // 积分 --------------------- END
            ...
            // 判断上下架的状态，只显示上架商品     章锐2016/6/24 --------------------- BEGIN
		...
		if(!empty($itemIds)){
			$instockIds = $db -> executeQuery('select item_id from sysitem_item_status where item_id in ('.$itemIds.') and approve_status ="instock"')->fetchAll();
			foreach($instockIds as $key => $value){
				$instocks[] = $value['item_id'];
			}
			foreach($items as $key => $value){
				foreach($value as $keys => $values){
					if(in_array($values['item_id'], $instocks)){
						unset($items[$key][$keys]);					
					}
				}
			}
		...
		// 判断上下架的状态，只显示上架商品     章锐2016/6/24 --------------------- END
		//商品所属分类  章锐2016/6/28   -----------------------------BEGIN
		foreach($items as $key => $value){
			foreach($value as $keys => $values){
				if(!empty($values['shop_cat_id'])){
					$cat[$key]=str_replace(',', '', $values['shop_cat_id']);
					break;
				}
			}
		}
		$pagedata['cat'] = $cat;   //分类id
    //商品所属分类  章锐2016/6/28   -----------------------------END
    ...
    public function search()
    {
        ...
        if($itemsList['list'])
        {
            ...
            for ($i = 0; $i < $_tmp_count; $i++) {
                $itemsList['list'][$i]['price_deposit'] = $itemsList['list'][$i]['price'] * 100;
            }
            ...
            if($activityItemList['list'])
            {
                foreach($activityItemList['list'] as $key=>$value)
                {
                    ...
                    $itemsList['list'][$value['item_id']]['price_deposit'] = $itemsList['list'][$value['item_id']]['price'] * 100;
                }
            }
        }
    ...
    // 更多页判断上下架的状态，只显示上架商品     章锐2016/6/24 --------------------- END
		...
		if(!empty($itemIds)){
			...
			foreach($items as $key => $value){
				if(in_array($value['item_id'], $instocks)){
					unset($items[$key]);					
				}
			}
