...
class topc_ctl_member_aftersales extends topc_ctl_member{
    ...
    public function commitAftersalesApply()
    {
        ...
        if($postdata['aftersales_type']=="REFUND_GOODS"){
        	$res="RETURN";
        }else{
        	$res="EXCHANGE";
        }
        $db = app::get('topc')->database();
		$result= $db->executeQuery('select tid,num from systrade_order where oid='.$postdata['oid'])->fetch();  
		$db->executeQuery('update systrade_order set aftersales_num='.$result['num'].' where oid='.$postdata['oid']);    
		$re= $db->executeQuery('select cancel_reason from systrade_trade where tid='.$result['tid'])->fetch();   
		$reason=$re['cancel_reason'].'<br/>'.date('Y-m-d H:i:s').'&nbsp;&nbsp;'.$postdata['description'];
		$db->executeQuery('update systrade_trade set order_status="'.$res.'",cancel_reason="'.$reason.'"  where tid='.$result['tid']);    
        if ($validator->fails())
        {
    ...
    public function sendback()
    {
        ...
        try
        {
            ...
            $validator = validator::make(
                    ...
                    ['required','required','required|mobile','required'],
                    ['物流公司不能为空!','物流单号不能为空!','收货手机号不能为空!|收货手机号格式不对!','收货地址不能为空!']
                );
            $validator->newFails();
