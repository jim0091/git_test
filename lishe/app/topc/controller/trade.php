...
class topc_ctl_trade extends topc_controller{
    ...
    public function create()
    {
        ...
        // 将comid写入数据表  start  20160708        
        $db=app::get('systrade_trade')->database();
        $comIdArr = $db->executeQuery("select comId from sysuser_user_deposit where user_id=".$postData['user_id'])->fetch();
        $comId=$comIdArr['comId'];
        $resTrade = $db->executeQuery("select tid from ectools_trade_paybill where payment_id=".$paymentId)->fetchAll();
        if($resTrade){
        	foreach($resTrade as $key=>$value){
				$tid[]=$value['tid'];
			}
             
             $skuInfo=$db->executeQuery("select a.tid,b.cost_price from systrade_order as a left join sysitem_sku as b on a.sku_id=b.sku_id where a.tid IN (".implode(',',$tid).")")->fetchAll();
             if($skuInfo){
                 foreach($skuInfo as $key1=>$value1){
                    $db->executeUpdate("update systrade_order set cost_price =".$value1['cost_price']." where tid = ".$value1['tid']);
                 }
            }
			$db->executeUpdate("update systrade_trade set com_id='".$comId."' where tid IN (".implode(',',$tid).")");
        }
       // 将comid写入数据表  end  20160708
