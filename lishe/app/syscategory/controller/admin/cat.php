...
class syscategory_ctl_admin_cat extends desktop_controller {
    ...
    public function index()
    {
        ...
        if($tree)
        {
            foreach($tree as $k=>$v)
            {
                ...
                // 将 jd_cid 加入到数组中 2016-6-12 start
                $con = mysql_connect('127.0.0.1','bbc','bbc@1234');
              	$db = mysql_select_db('bbc',$con);
                $cat_id = $tree[$k]['cat_id'];
                $sql_jd = '';
                $sql_jd = 'select jd_cid from syscategory_cat where cat_id = '.$cat_id;
                $query_jd = mysql_query($sql_jd);
                $res_jd = mysql_fetch_assoc($query_jd);
                // echo $res_jd['jd_cid'];
                // echo '<br/>';
                $tree[$k]['jd_cid'] = $res_jd['jd_cid'];

                mysql_close($con);
                // echo '<br/>';


                 // 将 jd_cid 加入到数组中 2016-6-12 end
            }
        }
    ...
    public function add($nCatId = 0)
    {
        ...
        $catInfo = $objMdlCat->getRow('cat_path, level, cat_name, cat_id, parent_id, jd_cid', array('cat_id'=>$nCatId));
        ...
        if($catInfo['level']=='2')
        {
            ...
            $level1CatInfo = $objMdlCat->getRow('cat_path, level, cat_name, cat_id, parent_id, jd_cid', array('cat_id'=>$catInfo['parent_id']));
        ...
        // 20160608 新增开始
        $pagedata['cat']['jd_cid'] = $catInfo['jd_cid'];
        // 20160608 新增结束
    ...
    public function edit($nCatId, $is_leaf=0)
    {
        ...
        $catInfo = $objMdlCat->getRow('jd_cid, cat_path, level, cat_name, cat_logo, cat_id, parent_id, guarantee_money, platform_fee, cat_service_rates, order_sort,cat_template', array('cat_id'=>$nCatId));

        // 组织上级分类数据，1级分类显示无，2级分类显示1级分类,3级分类则显示对应的2级和1级分类
        $catList[0] = array('cat_id'=>0, 'cat_name'=>app::get('syscategory')->_('----无----'), 'step'=>1);
        if($catInfo['level']=='2')
        {
            $level1CatInfo = $objMdlCat->getRow('jd_cid, cat_path, level, cat_name, cat_id, parent_id, guarantee_money, platform_fee, cat_service_rates, order_sort', array('cat_id'=>$catInfo['parent_id']));
            $catList[1] = array('cat_id'=>$level1CatInfo['cat_id'], 'cat_name'=>$level1CatInfo['cat_name'], 'step'=>1);
        }
        if($catInfo['level']=='3')
        {
            $level2CatInfo = $objMdlCat->getRow('jd_cid, cat_path, level, cat_name, cat_id, parent_id, guarantee_money, platform_fee, cat_service_rates, order_sort', array('cat_id'=>$catInfo['parent_id']));
            $level1CatInfo = $objMdlCat->getRow('jd_cid, cat_path, level, cat_name, cat_id, parent_id, guarantee_money, platform_fee, cat_service_rates, order_sort', array('cat_id'=>$level2CatInfo['parent_id']));
            $catList[1] = array('cat_id'=>$level1CatInfo['cat_id'], 'cat_name'=>$level1CatInfo['cat_name'], 'step'=>1);
            $catList[2] = array('cat_id'=>$level2CatInfo['cat_id'], 'cat_name'=>$level2CatInfo['cat_name'], 'step'=>2);
        }
        $pagedata['catList'] = $catList;
        
        $pagedata['cat']['jd_cid'] = $catInfo['jd_cid']; //新增
        ...
        $pagedata['cat']['act'] = $_GET['act'];
        $pagedata['cat']['p'] = $_GET['p'][0];
    ...
    public function save()
    {
        ...
        $postData = $_POST;
         ...
         // $postData['cat']['jd_cid'] = intval($_POST['cat']['jd_cid']); 
        ...
        else
        {
            ...
            // echo '<br/>';
            // echo '<br/>';
           //  // var_dump($postData['cat']['jd_cid']);
           // var_dump($postData['cat']);

           if(isset($postData['cat']['cat_id']) && $postData['cat']['cat_id'] != ''){
             // echo "1111";
             // exit;
              $cat_id = intval($postData['cat']['cat_id']);
              $jd_cid = intval($postData['cat']['jd_cid']);

              $con = mysql_connect('127.0.0.1','bbc','bbc@1234');
              $db = mysql_select_db('bbc',$con);
              // var_dump($con,$db);
            
              $sql = 'update syscategory_cat set jd_cid = '.$jd_cid.' where cat_id = '.$cat_id;
