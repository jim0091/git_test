...
class systrade_finder_trade{
    ...
    public function detail_items($tradeId)
    {
        ...
        $ItemInfo=$db->executeQuery("select item_id,cost_price from sysitem_item where item_id in(".implode(',',$itemArr).")")->fetchAll();
        if($ItemInfo){
            foreach($orders as $k1=>$v1){
                foreach($ItemInfo as $k2=>$v2){
                    if($v1['item_id']==$v2['item_id']){
                        $orders[$k1]['cost_price']=$v2['cost_price'];
                        break;
                    }
                }
            }
    ...
    public function detail_users($tradeId)
    {
        ...
        // 20160819 开始
        $userId=intval($users['userId']);
        $db = app::get('sysuser_user_deposit')->database();
        $depositInfo=$db->executeQuery("select comId,comName from sysuser_user_deposit where user_id = ".$userId)->fetch();
        $users['comId']=trim($depositInfo['comId']);
        $users['comName']=trim($depositInfo['comName']);
        // 20160819 结算
        $pagedata['user'] = $users;
