   ...
   /**
    ...
    public function getTradeTotal($tradeParams, $aCart)
    {
        foreach ($aCart['resultCartData'] as $shopId => $shopCartData )
        {
				...
				if(!empty($res)){
					foreach($res as $key => $value){
		          	...
		          	if(!empty($freepostageIdArr)){
						...
						$result= $db->executeQuery('select condition_type,limit_money from syspromotion_freepostage where freepostage_id in('.$freepostageId.')')->fetchAll();
						...
						if($totalInfo['payment']>=$freemoney){
			...
			/**
            if($shopId==10 && $totalInfo['total_fee']>=128){
				$totalInfo['payment']=$totalInfo['payment']-$totalInfo['post_fee'];
				$totalInfo['post_fee']=0;
			}
			* **/
    ...
    private function _chgdata( $tradeParams, $aCart=array() )
    {
        ...
        foreach ($aCart['resultCartData'] as $shopId => $shopCartData )
        {
            ...
            $shopTradeData = [
                ...
                'dlytmpl_id' => $tradeParams['shipping'][$shopId]['template_id'],
