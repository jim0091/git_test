...
class search_policy_sphinx implements search_interface_policy {
    ...
    public function getIndexParams($indexName)
    {
        ...
        $data['setting']['max_matches'] = empty($setting['max_matches']) ? '1000' : $setting['max_matches'];//默认分页数目
    ...
    public function _sphinxql($cols='*', $offset=0, $limit=100000, $orderBy=null, $groupBy='')
    {
        ...
        $option = array('ranker'=>$setting['ranker'],'max_matches'=>$setting['max_matches']);
        if(!empty($option))
        {
            $optionStr = ' OPTION ';
