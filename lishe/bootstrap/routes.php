...
route::group(array('middleware' => ['topc_middleware_redirectIfFromWap', 'theme_middleware_preview']), function() {
    ...
    */
    route::group(array(), function() {
        ...
        # 商城首页
        //route::get('/shop.html', [ 'as' => 'topc', 'uses' => 'topc_ctl_default@shop']);
        //这里做了一个跳转口，用于登陆等需要跳转的地方，可以跳转post等方式
        route::get('/redirect.html', [ 'as' => 'topc', 'uses' => 'topc_ctl_default@redirect']);
        //----------
        # 商品收藏
        ...
        # 积分均价
        route::get('integral_average.html', [ 'uses' => 'topc_ctl_list@integral_average' ]);
        # 迷礼圈首页
        route::get('circle.html', [ 'uses' => 'topc_ctl_list@circle' ]);
        # 迷礼圈专题页
        route::get('circle_detail.html', [ 'uses' => 'topc_ctl_list@circle_detail' ]);

        # 登陆
        route::get('/api/index', [ 'uses' => 'topc_ctl_user@index' ]);
        # 调取获取公共密钥接口 sys/getPublicKey
        route::get('/api/sys/getPublicKey', [ 'uses' => 'topc_ctl_user@getPublicKey' ]);
        # 登录接口 mall/login
        route::get('/api/mall/login', [ 'uses' => 'topc_ctl_user@mallLogin' ]);

        # 礼舍首页
        route::get('theme.html', ['uses' => 'topc_ctl_default@theme_index']);
        # 礼舍首页-5大角色二级页面-礼舍用户
        route::get('theme/user.html', ['uses' => 'topc_ctl_default@theme_user']);
        # 礼舍首页-5大角色二级页面-hr
        route::get('theme/hr.html', ['uses' => 'topc_ctl_default@theme_hr']);
        # 礼舍首页-5大角色二级页面-boss
        route::get('theme/boss.html', ['uses' => 'topc_ctl_default@theme_boss']);
        # 礼舍首页-5大角色二级页面-员工
        route::get('theme/member.html', ['uses' => 'topc_ctl_default@theme_member']);
        # 礼舍首页-5大角色二级页面-新手入门
        route::get('theme/new-hand.html', ['uses' => 'topc_ctl_default@theme_new_hand']);

        # 礼舍首页-伙伴怎么说
        route::get('theme/evaluate/index.html', ['uses' => 'topc_ctl_default@theme_evaluate']);
        # 礼舍首页-伙伴怎么说详情
        route::get('theme/evaluate/detail.html', ['uses' => 'topc_ctl_default@theme_evaluate_detail']);

        # 礼舍首页-礼舍能做什么
        route::get('theme/help.html', ['uses' => 'topc_ctl_default@theme_help']);

        # 一企一舍API-注册用户
        route::get('api/yqys/register', ['uses' => 'topc_ctl_passport@LSApiCreate']);
        route::get('api/yqys/test/register', ['uses' => 'topc_ctl_passport@LSApiCreateTest']);

        # 商城会员-激活成为一企一舍用户-1st
        route::get('user/yqys/activation-1st.html', ['uses' => 'topc_ctl_passport@activationFir']);
        # 商城会员-激活成为一企一舍用户-2st
        route::get('user/yqys/activation-2st.html', ['uses' => 'topc_ctl_passport@activationSec']);
        # 商城会员-激活成为一企一舍用户-SUCCESS
        route::get('user/yqys/activation-success.html', ['uses' => 'topc_ctl_passport@activationSuccess']);

        # 商城会员-激活-注册用户
        route::get('api/yqys/activation/register.html', ['uses' => 'topc_ctl_passport@createWithActivation']);

        #心意攻略-01
        route::get('topic-01.html', [ 'uses' => 'topc_ctl_default@topic01' ]);
        #心意攻略-02
        route::get('topic-02.html', [ 'uses' => 'topc_ctl_default@topic02' ]);
        #心意攻略-03
        route::get('topic-03.html', [ 'uses' => 'topc_ctl_default@topic03' ]);
        #心意攻略-04
        route::get('topic-04.html', [ 'uses' => 'topc_ctl_default@topic04' ]);
        #心意攻略-05
        route::get('topic-05.html', [ 'uses' => 'topc_ctl_default@topic05' ]);
        #心意攻略-06
        route::get('topic-06.html', [ 'uses' => 'topc_ctl_default@topic06' ]);
        #心意攻略-07
        route::get('topic-07.html', [ 'uses' => 'topc_ctl_default@topic07' ]);
        #心意攻略-08
        route::get('topic-08.html', [ 'uses' => 'topc_ctl_default@topic08' ]);
        #心意攻略-09
        route::get('topic-09.html', [ 'uses' => 'topc_ctl_default@topic09' ]);

        #赠送对象
        route::get('given-object.html', [ 'uses' => 'topc_ctl_default@givenObject' ]);
        #赠送情景
        route::get('given-scene.html', [ 'uses' => 'topc_ctl_default@givenScene' ]);
        #TA喜欢
        route::get('TA-like.html', [ 'uses' => 'topc_ctl_default@TALike' ]);

        #帮助中心-01
        route::get('help_index.html', [ 'uses' => 'topc_ctl_default@help01' ]);
        #帮助中心-02
        route::get('help_summary.html', [ 'uses' => 'topc_ctl_default@help02' ]);
        #帮助中心-03
        route::get('help_resume.html', [ 'uses' => 'topc_ctl_default@help03' ]);
        #帮助中心-04
        route::get('help_problem.html', [ 'uses' => 'topc_ctl_default@help04' ]);
        #帮助中心-05
        route::get('help_recruitment.html', [ 'uses' => 'topc_ctl_default@help05' ]);
        #帮助中心-07
        route::get('help_partner.html', [ 'uses' => 'topc_ctl_default@help06' ]);
        #帮助中心-08
        route::get('help_we.html', [ 'uses' => 'topc_ctl_default@help07' ]);

    });
    route::group(array(), function() {
    	
        route::get('raiders.html', [ 'uses' => 'topc_ctl_raiders@index' ]);
    ...
    */
    route::group(array('middleware' => 'topc_middleware_authenticate'), function() {
        ...
        route::post('member-tradelist.html', [ 'as' => 'home', 'uses' => 'topc_ctl_member@tradeStatusList' ]);
        # 会员中心个人资料
        ...
        route::get('activity-trade-list.html', [ 'uses' => 'topc_ctl_member_trade@activityTradeList' ]);
        ...
        route::match(array('GET', 'POST'), 'recharge-doPay-YQYS.html', ['uses' => 'topc_ctl_member_deposit@doRechargeWithYQYS']); // YQYS 积分查询-更新积分
    ...
    */
    route::group(array('middleware' => 'topc_middleware_authenticate'), function() {
        ...
        route::match(array('GET', 'POST'),'finish-deposit.html', [ 'uses' => 'topc_ctl_paycenter@finishWithDeposit' ]);
    });
 });
...
route::group(array('prefix' => 'shop','middleware' => 'topshop_middleware_permission'), function() {
    ...
    # 首页
    route::get('/admin.html', [ 'as' => 'topshop.home', 'uses' => 'topshop_ctl_index@index' ]);
    ...
    #组合促销
    route::post('promotion/packagelist.html', [ 'as' => 'topshop.promotion.packagelist', 'uses' => 'topshop_ctl_promotion_package@searchItem' ]);
...
route::group(array('prefix' => 'utils'), function() {
    ...
    route::get('vcode.html', [ 'as' => 'toputil.vcode', 'uses' => 'toputil_ctl_vcode@gen_vcode' ]);
    route::get('check_vcode.html', [ 'as' => 'toputil.vcode', 'uses' => 'toputil_ctl_vcode@check_vcode' ]);
