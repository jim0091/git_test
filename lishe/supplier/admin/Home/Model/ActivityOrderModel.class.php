 ...
 */
    ...
    class ActivityOrderModel extends CommonModel{
    	public function __construct(){
			...
			public function getOrderItemsToNum($condition){
			  ...
			  return $this->modelAorder->where($condition)->order('order_id desc')->getField('atid',TRUE);
			}	
			//取出符合的订单号
			public function getConditionOrderTid($condition){
			  return $this->modelAtrade->where($condition)->order('creat_time desc')->getField('atid',TRUE);
			}			
			//订单	
			public function getOrder($condition,$limit){
				return M('')->table('company_activity_trade t')->field('atid,com_id,receiver_name,receiver_state,receiver_city,receiver_district,receiver_address,receiver_mobile,creat_time,payment,payed_fee,pay_time,status,pay_type,order_status,refund_fee,buyer_message')->where($condition)->order('creat_time desc')->limit($limit)->select();
			}		
			//订单中的所有商品
	...
	public function getOrderDetail($tid,$adminId){
		$res=$this->modelAtrade->where('atid='.$tid)->find();
		$info=$this->modelAorder->where('atid='.$tid.' and supplier_id ='.$adminId.' and send_type=2')->select();
		$res['more']=$info;
		return $res;
	}		
	//取出该信息的物流信息
