...
<div class="main-wrap">
    ...
    <div class="crumb-wrap">
        <div class="crumb-list"><i class="icon-font"></i><a href="index.html">首页</a><span class="crumb-step">&gt;</span><span class="crumb-name">待发货订单</span></div>
    </div>
            ...
            <div class="result-content">
                <table class="result-tab" min-width="100%">
                    ...
                    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td><?php echo ($val['warehouse_id']); ?></td>
                            <td><?php echo ($val['remarks']); ?></td>
                            <td><?php echo ($val['build_people']); ?></td>
                            <td><?php echo ($val['build_time']); ?></td>
                            <td><a href="javascript::" onclick="layer_show('查看','<?php echo U('Home/orderDetail/',array('order_id'=>$val['order_id']));?>','1','800','500')">详情 </a><a href="javascript::" onclick="sendOrder('<?php echo ($val['order_id']); ?>','1')"> 发货 </a><a onclick="checkOrder('<?php echo ($val['order_id']); ?>','-1')" href="javascript::">  </a></td>
...
<script>
    ...
    function checkOrder(id,status){
        layer.confirm('确认审核？', {
            ...
            $.post("<?php echo U('Home/checkOrder/');?>",data,function(text){
                if(text=="1"){
                    layer.msg("成功");
                }else{
                    layer.msg("审核失败");
    ...
    function sendOrder(id){
        layer.confirm('确认发货？', {
            ...
            layer.prompt({title: '如有需要请填写备注', formType: 2}, function(text){
                ...
                $.post("<?php echo U('Home/sendOrder/');?>",data,function(res){
                    if(res=="1"){
                        layer.msg("成功");
                    }else{
                        layer.msg("发货失败");
