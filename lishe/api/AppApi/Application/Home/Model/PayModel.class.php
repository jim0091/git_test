...
class PayModel extends Model{
	...
	public function getTradeList($condition){
		if (!$condition) {
			...
			return $this->modelTrade->where($condition)->field('created_time')->select();
		}		
	}
	//根据tid查询订单子表
	public function getOrderList($condition){
		if (!$condition) {
			...
			$orderList = $this->modelTradeOrder->table('systrade_order o,sysitem_item_status s')->where('o.item_id = s.item_id '.$condition)->field('o.item_id,o.sku_id,s.approve_status')->select();
			if (!$orderList) {
				return false;
			}else{
				...
				$itemSkuList = $this->modelSku->where('sku_id in('.implode(',',$skuIds).')')->field('item_id,price')->select();
				foreach ($orderList as $key => $value) {
